...
**Until this is fixed structurally, any regeneration needs**

    --exclude-path _view.f90

which was measured to restore exactly 69 units, none added, none removed.

**The structural fix belongs in the scanner, not in the flag.** All four
generated files carry `! Auto-generated by VIT (Verified Incremental Translator)`
on line 1, so the derivation can refuse them on content rather than relying on
an operator remembering a path fragment. A flag that must be remembered is a
flag that will be forgotten, and the failure is silent when it is.
