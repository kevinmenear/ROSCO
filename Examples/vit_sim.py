...
# VIT replication: resolve libdiscon.so from THIS tree, not from the
# installed `rosco` package.
#
# The package is installed editable from the original ROSCO checkout, so
# `from rosco import discon_lib_path` returns that tree's library — the
# original campaign's finished pure-C++ libdiscon.so. Left alone, every
# scenario here would exercise the completed migration and pass, while
# appearing to test the replication. This is the one failure mode that
# produces a green result and no signal at all.
#
# The Python toolbox below still comes from the installed package. That is
# fine: it is simulation glue, not translated code.
import os as _os
_repl_lib = _os.path.join(
    _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))),
    "rosco", "lib", "libdiscon.so")
if _os.path.exists(_repl_lib):
    lib_name = _repl_lib
else:
    raise RuntimeError(
        f"Replication libdiscon.so not found at {_repl_lib}. Build it first; "
        "do not fall back to the installed package, which points at the "
        "original ROSCO tree.")
