...
> **This is the `integrated` branch.** It contains verified C++ translations of 39 ROSCO controller functions, integrated into the Fortran codebase. The mixed Fortran/C++ `libdiscon.so` produces byte-identical simulation output to the original pure-Fortran version across 9 scenarios (672,000 float64 values compared). Development happens on `main` — this branch is a point-in-time snapshot for inspection.
>
> **What you'll see in the source files:**
> - **`rosco/controller/src/*.cpp`** — C++ implementations with `extern "C"` wrappers, generated by [VIT](https://github.com/NatLabRockies/vit). Each file includes `vit_types.h` (C struct definitions matching the Fortran derived types) and contains the translated algorithm plus a callable entry point.
> - **`rosco/controller/src/*.f90`** — Fortran source files where translated functions have been replaced with thin wrappers. The wrapper has the same signature as the original function but delegates to the C++ `_c` entry point via `ISO_C_BINDING`. Callers are unaffected — they still call the same Fortran function name.
> - **`rosco/controller/src/vit_types.h`** — C struct definitions for Fortran derived types (`LocalVariables`, `ControlParameters`, etc.), auto-generated from `ROSCO_Types.f90`.
> - **`rosco/controller/src/vit_translated.h`** — `extern "C"` declarations for all 39 translated functions, enabling C++ functions to call each other directly.
> - **`rosco/controller/src/vit_*_view.f90`** — Fortran modules that populate "view structs" for types with ALLOCATABLE arrays (`ControlParameters`, `PerformanceData`). These convert Fortran's dynamic arrays to C-compatible pointer+size pairs at runtime.
>
> The original C++ translations (implementation-only, without wrappers) are in `translations/`. The Fortran source and translations on `main` are the authoritative versions.
