...
**As of 2026-08-10: unit #1 `AddToList` is `integrated` and CLOSED.** The
prediction that its signature cannot cross the C bridge is **REFUTED** — the
first dispatch refuted the basis, this one refutes the verdict. An
`ALLOCATABLE, INTENT(INOUT)` dummy now crosses as a Fortran 2018 C descriptor,
generated by VIT (`37f8bdf`) and understood by the loop's harness (`cf885e3`);
nothing about the integration is hand-written.

That is escalation 1, answered by doing it. **Escalation 2 stands and is
sharper for having been avoided here.** This unit closed because it acquired a
differential harness and a mutation score — instruments that do not need the
gate. The gate still cannot see it: perturbing the integrated C++ moves 0 of
5,252,000 values. A dead unit whose signature does NOT cross still has no path
to any of the three, and `loop/done.py` still has no branch for
`integrated_unexercised`.
...
2 attempted / 2 integrated / 0 integrated_unexercised / 0 out_of_scope /
