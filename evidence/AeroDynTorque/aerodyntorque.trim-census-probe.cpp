...
struct TrimCensus {
    long calls = 0, with_trailing_blank = 0, all_blank = 0, empty_view = 0;
    long n_le_0 = 0, n_eq_1 = 0;
    int nmin = 1 << 30, nmax = -(1 << 30);
    ~TrimCensus() {
        std::fprintf(stderr,
            "TRIMCENSUS calls=%ld n_le_0=%ld n_eq_1=%ld n_min=%d n_max=%d "
            "empty_view=%ld with_trailing_blank=%ld all_blank=%ld\n",
            calls, n_le_0, n_eq_1, nmin, nmax, empty_view,
            with_trailing_blank, all_blank);
    }
};
TrimCensus g_trim;
...
std::string errmsg_trim(const errorvariables_view_t* ErrVar) {
    ...
    g_trim.calls++;
    if (n <= 0) g_trim.n_le_0++;
    if (n == 1) g_trim.n_eq_1++;
    if (n < g_trim.nmin) g_trim.nmin = n;
    if (n > g_trim.nmax) g_trim.nmax = n;
    if (v.empty()) g_trim.empty_view++;
    const size_t cut = v.find_last_not_of(' ') + 1;
    if (cut < v.size()) g_trim.with_trailing_blank++;
    if (!v.empty() && cut == 0) g_trim.all_blank++;
    return std::string(v.substr(0, cut));
}
