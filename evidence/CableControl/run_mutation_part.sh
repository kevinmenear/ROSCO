#!/bin/bash
# ONE operator-filtered mutation part for unit #49 `CableControl`, guarded.
#
#   bash scripts/run_if_time_remains.sh 900 \
#       bash evidence/CableControl/run_mutation_part.sh <part-name> <operator>[,<operator>...] [--equivalences]
#
# Copied from evidence/AeroDynTorque/run_mutation_part.sh (unit #48) with the
...
# harness's Fortran side calls the wrapper, which calls `cablecontrol_c`, which
# links the harness's own compiled copy -- so both sides would be the mutant, and
# `vit_mutate.py` refuses that configuration outright. And all three of this
# unit's CALLEES are themselves integrated, so on an integrated tree the
# reference side's interp1d, SecLPFilter_Vel and PIController are the C++ ones as
# well.
#
...
# `harness/CableControl.json` was green on -- which is unit #26's rule.
set -uo pipefail
...
EQ=""
[ "${3:-}" = "--equivalences" ] && \
    EQ="--equivalences /workspace/ROSCO-r2/mutation/CableControl.equivalences.json"

OPARGS=""
IFS=, read -ra _ops <<< "$OPS"
for o in "${_ops[@]}"; do OPARGS="$OPARGS --operator $o"; done

bash scripts/mutate_guarded.sh translations/Controllers/cablecontrol.cpp \
    docker exec vit-dev bash -lc \
      "cd /workspace/ROSCO-r2 && python3 /workspace/translation-loop/scripts/vit_mutate.py \
         CableControl --root /workspace/ROSCO-r2 \
         --cpp translations/Controllers/cablecontrol.cpp --module Controllers \
         $OPARGS $EQ --out mutation/CableControl.clean.$PART.json"
