...
void ChkParseData(char* Words, int len_Words, char* ExpVarName, int len_ExpVarName, char* FileName, int len_FileName, int FileLineNum, errorvariables_view_t* ErrVar) {
    ...
    // PROBE, not a translation: count the cases in which the corpus makes the
    // two elements of `Words` DIFFER. `ErrVar%ErrStat` is compared by the
    // harness (R4 lists it) and the reference never writes it, so every case
    // this touches fails and no case it does not touch can fail. The failure
    // count IS the number of cases with distinct elements.
    if (std::memcmp(word(Words, len_Words, 1), word(Words, len_Words, 2),
                    static_cast<std::size_t>(len_Words)) != 0) {
        ErrVar->ErrStat += 1;
    }
