...
void ChkParseData(char* Words, int len_Words, char* ExpVarName, int len_ExpVarName, char* FileName, int len_FileName, int FileLineNum, errorvariables_view_t* ErrVar) {
    ...
    // PROBE, not a translation: count the cases in which Words(2) IS the
    // expected name, by the reference's own test -- truncate to 20, uppercase,
    // TRIM-compare. `ErrVar%ErrStat` is compared by the harness and the
    // reference never writes it, so the failure count IS that number. It is the
    // arithmetic the wrapper red test needs: a transposition of the two words
    // can only be seen where exactly one of them matches.
    {
        char vit_w2[UCVarNameLen], vit_ex[UCVarNameLen];
        char_assign(vit_w2, UCVarNameLen, word(Words, len_Words, 2), len_Words);
        char_assign(vit_ex, UCVarNameLen, ExpVarName, len_ExpVarName);
        conv2uc_c(vit_w2, UCVarNameLen);
        conv2uc_c(vit_ex, UCVarNameLen);
        if (ftrim(vit_w2, UCVarNameLen) == ftrim(vit_ex, UCVarNameLen)) {
            ErrVar->ErrStat += 1;
        }
    }
