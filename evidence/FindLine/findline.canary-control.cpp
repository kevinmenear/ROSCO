...
void FindLine(char* FileLines, int n_FileLines, int len_FileLines, char* ParamName, int len_ParamName, int32_t* FoundLine, char* Line, int* LineNum, int has_AryLen, int AryLen) {
    ...
    char ParamNameUC[MaxParamLength + 1];
    ParamNameUC[MaxParamLength] = '\x7f';   // CANARY
    char_assign(ParamNameUC, MaxParamLength, ParamName, len_ParamName);
    conv2uc_c(ParamNameUC, MaxParamLength);
    ParamNameUC[MaxParamLength] = 'X';   // CONTROL: unconditionally disturbs the canary
    ...
    char FileLineUC[MaxParamLength + 1];
    FileLineUC[MaxParamLength] = '\x7f';   // CANARY
    ...
    *LineNum = 0;
    ...
    // CANARY CHECK. Both mutants declare these arrays one byte LONGER; the
    // claim is that the extra byte is never read or written, so the two
    // programs are the same program. That claim is made an OUTPUT here --
    // LineNum is compared by R4 and the reference writes only 0 or I, so -7
    // cannot collide with a real answer.
    if (ParamNameUC[MaxParamLength] != '\x7f' || FileLineUC[MaxParamLength] != '\x7f') {
        *LineNum = -7;
    }
