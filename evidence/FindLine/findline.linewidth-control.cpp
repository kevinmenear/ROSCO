// CONTROL for the two zeros that classify mutation survivor ca75abea
// (`const_tweak '2048' -> '2049'`). That mutant is invisible to the
// differential harness (0 of 2514) and to the gate (0 of 5,252,000), and a
// zero is worth nothing until the probe is shown able to be non-zero (P10) --
// this campaign has already had one control that was a NO-OP and reported the
// probe's own number.
//
// So this is the SAME constant at the SAME site moved by one byte in the
// OTHER direction: 2048 -> 2047 leaves the last byte of `Line` as the caller
// handed it in rather than blank-filling it. That byte is INSIDE the buffer
// the harness compares, so it must fail -- and the asymmetry is the whole
// finding: one byte too few is a wrong answer, one byte too many is not an
// answer at all.
...
constexpr int MaxLineLength = 2047;
...
void FindLine(char* FileLines, int n_FileLines, int len_FileLines, char* ParamName, int len_ParamName, int32_t* FoundLine, char* Line, int* LineNum, int has_AryLen, int AryLen) {
    ...
    conv2uc_c(ParamNameUC, MaxParamLength);
