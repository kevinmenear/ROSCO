// MUTANT ca75abea, written out so an instrument that is not `vit_mutate.py`
// can be pointed at it. `const_tweak '2048' -> '2049'` on MaxLineLength: the
// one mutant of this unit that survived a 2514-case differential harness AND a
// 5,252,000-value gate, because char_assign then writes one byte past the
// CALLER's 2048-byte buffer and no value comparison reads outside the buffer it
// compares. Used only by run_asan_demo.sh.
...
constexpr int MaxLineLength = 2049;
