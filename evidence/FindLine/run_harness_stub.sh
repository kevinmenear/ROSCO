...
#
#   bash evidence/FindLine/run_harness_stub.sh <stub.cpp> <out.json> "<what was cut>"
#
...
# `translations/ROSCO_Helpers/findline.cpp` to `findline.hpp`, so
# swapping that file is the whole of the lever.
#
# CARRIED VERBATIM from evidence/ChkParseData/run_harness_stub.sh with the unit
# and stem substituted -- the hash-verify loop, the trap-restore and the
# clean-tree requirement are that unit's, not re-derived (P4).
#
