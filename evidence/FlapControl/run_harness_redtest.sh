#!/bin/bash
# ONE differential-harness red test per invocation, for unit #50 `FlapControl`.
#
#   bash scripts/run_if_time_remains.sh 600 \
#       bash evidence/FlapControl/run_harness_redtest.sh <stub.cpp> <out.json> "<what was perturbed>"
#
# Copied from evidence/CableControl/run_harness_redtest.sh (unit #49) with the
...
# `harness/FlapControl.json`'s count, the two results are not about one corpus
# and the difference is the finding.
#
# RUN IT ON THE CLEAN TREE. All FIVE of this unit's callees -- ColemanTransform,
# ColemanTransformInverse, PIController, PIIController and saturate -- ARE
...
CPP=translations/Controllers/flapcontrol.cpp
...
cp "$E/$1" "$CPP"
bash scripts/harness.sh FlapControl Controllers flapcontrol \
