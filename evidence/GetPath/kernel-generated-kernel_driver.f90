    ...
    PROGRAM kernel_driver 
        ...
        INTEGER(KIND=intki) :: accinfile_size 
        CHARACTER(LEN=:), DIMENSION(:), ALLOCATABLE :: accinfile 
        myrank = 0 
        mpisize = 1 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 GetPath.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    READ (UNIT = kgen_unit) accinfile_size 
                    CALL kr_kgen_readcontrolparameterfilesub_subp1(accinfile, kgen_unit, "accinfile", .FALSE.) 
                      
                    !extern input variables 
                      
                    !callsite part 
                    CALL readcontrolparameterfilesub(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, accinfile_size, &
                    &accinfile) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: GetPath" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: GetPath: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: GetPath: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
        ...
        !read state subroutine for kr_kgen_readcontrolparameterfilesub_subp1 
        SUBROUTINE kr_kgen_readcontrolparameterfilesub_subp1(var, kgen_unit, printname, printvar) 
            CHARACTER(LEN=:), INTENT(INOUT), ALLOCATABLE, DIMENSION(:) :: var 
            ...
            IF (kgen_istrue) THEN 
                IF (ALLOCATED( var )) THEN 
                    DEALLOCATE (var) 
                END IF   
                READ (UNIT = kgen_unit) kgen_len 
                READ (UNIT = kgen_unit) kgen_bound(1, 1) 
                READ (UNIT = kgen_unit) kgen_bound(2, 1) 
                ALLOCATE (CHARACTER(LEN=kgen_len) :: var(kgen_bound(1,1):kgen_bound(2,1))) 
                READ (UNIT = kgen_unit) var 
                IF (PRESENT( printvar ) .AND. printvar) THEN 
                    WRITE (*, *) "KGEN DEBUG: " // printname // " = ", var 
                END IF   
            END IF   
        END SUBROUTINE kr_kgen_readcontrolparameterfilesub_subp1 
