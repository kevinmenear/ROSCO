    ...
    !Generated at : 2026-08-11 11:19:55 
    !KGEN version : 0.8.1 
...
! VIT: removed USE discon (subroutine, not module)
        ...
        REAL(KIND=reki), DIMENSION(:), ALLOCATABLE :: avrswap 
        CHARACTER(KIND=c_char), DIMENSION(:), ALLOCATABLE :: avcoutname 
        myrank = 0 
        mpisize = 1 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 GetRoot.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    CALL kr_discon_real__reki_dim1(avrswap, kgen_unit, "avrswap", .FALSE.) 
                    CALL kr_discon_character__c_char_dim1(avcoutname, kgen_unit, "avcoutname", .FALSE.) 
                      
                    !extern input variables 
                      
                    !callsite part 
                    CALL discon(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, avrswap, avcoutname) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: GetRoot" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: GetRoot: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: GetRoot: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
        ...
        !read state subroutine for kr_discon_character__c_char_dim1 
        SUBROUTINE kr_discon_character__c_char_dim1(var, kgen_unit, printname, printvar) 
            CHARACTER(KIND=c_char), INTENT(INOUT), ALLOCATABLE, DIMENSION(:) :: var 
            INTEGER, INTENT(IN) :: kgen_unit 
            CHARACTER(LEN=*), INTENT(IN) :: printname 
            LOGICAL, INTENT(IN), OPTIONAL :: printvar 
            LOGICAL :: kgen_istrue 
            ...
            IF (kgen_istrue) THEN 
                ...
                END IF   
            END IF   
        END SUBROUTINE kr_discon_character__c_char_dim1 
