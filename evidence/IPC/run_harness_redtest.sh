#!/bin/bash
# ONE differential-harness red test per invocation, for unit #53 `IPC`.
#
#   bash scripts/run_if_time_remains.sh 900 \
#       bash evidence/IPC/run_harness_redtest.sh <stub.cpp> <out.json> "<what was perturbed>"
#
# Copied from evidence/ForeAftDamping/run_harness_redtest.sh (unit #52) with the
# unit and the stem changed. Its rules hold unchanged:
#
...
# `harness/IPC.json`'s count, the two results are not about one corpus and the
# difference is the finding.
#
# RUN IT ON THE CLEAN TREE. ALL SIX of this unit's callees -- ColemanTransform,
# ColemanTransformInverse, LPFilter, PIController, sigma and wrap_360 -- ARE
...
# it went ("keeping the <callee>_c bridge" on a clean tree).
#
...
cp "$E/$1" "$CPP"
bash scripts/harness.sh IPC Controllers ipc \
