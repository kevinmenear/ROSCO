// STUB, not the translation: the four arguments this campaign's 27 scenarios
// never vary at the live call site -- DT, omega, betaNum, betaDen -- replaced
// by the literals the input files carry (DT from the captured state,
// F_NotchFreqs=1.0 / F_NotchBetaNum=0.0 / F_NotchBetaDen=0.25 from Examples/*.IN).
// If the kernel PASSES this, it cannot constrain any of the four.
...
double NotchFilter(double InputSignal, double DT, double omega, double betaNum, double betaDen, filterparameters_t* FP, int iStatus, int32_t reset, int* inst, int has_InitialValue, double InitialValue) {
    ...
    const double K = 2.0 / 0.10000000149011612;  /* DT hardcoded */
    ...
    if ((iStatus == 0) || (reset != 0)) {
        ...
        // FP%nf_b2(inst) = (K**2.0 + 2.0*1.0*BetaNum*K + 1.0**2.0)/(K**2.0 + 2.0*1.0*BetaDen*K + 1.0**2.0)
        // FP%nf_b1(inst) = (2.0*1.0**2.0 - 2.0*K**2.0)  / (K**2.0 + 2.0*1.0*BetaDen*K + 1.0**2.0)
        // FP%nf_b0(inst) = (K**2.0 - 2.0*1.0*BetaNum*K + 1.0**2.0) / (K**2.0 + 2.0*1.0*BetaDen*K + 1.0**2.0)
        // FP%nf_a1(inst) = (2.0*1.0**2.0 - 2.0*K**2.0)  / (K**2.0 + 2.0*1.0*BetaDen*K + 1.0**2.0)
        // FP%nf_a0(inst) = (K**2.0 - 2.0*1.0*BetaDen*K + 1.0**2.0)/ (K**2.0 + 2.0*1.0*BetaDen*K + 1.0**2.0)
        //
        ...
        //    `std::pow`.
        //
        // 2. `**` binds TIGHTER than `*`, so `2.0*1.0**2.0` is
        //    `2.0 * (1.0*1.0)` and NOT `(2.0*1.0) * 1.0`. The two round
        //    differently. Every power below is parenthesised for that reason.
        //
        // 3. `2.0*1.0*BetaNum*K` is four factors at equal precedence, so it
        //    associates LEFT: `((2.0*1.0) * BetaNum) * K`. Written with the
        ...
        FP->nf_b2[i] = ((K * K) + 2.0 * 1.0 * 0.0 * K + (1.0 * 1.0)) / ((K * K) + 2.0 * 1.0 * 0.25 * K + (1.0 * 1.0));
        FP->nf_b1[i] = (2.0 * (1.0 * 1.0) - 2.0 * (K * K)) / ((K * K) + 2.0 * 1.0 * 0.25 * K + (1.0 * 1.0));
        FP->nf_b0[i] = ((K * K) - 2.0 * 1.0 * 0.0 * K + (1.0 * 1.0)) / ((K * K) + 2.0 * 1.0 * 0.25 * K + (1.0 * 1.0));
        FP->nf_a1[i] = (2.0 * (1.0 * 1.0) - 2.0 * (K * K)) / ((K * K) + 2.0 * 1.0 * 0.25 * K + (1.0 * 1.0));
        FP->nf_a0[i] = ((K * K) - 2.0 * 1.0 * 0.25 * K + (1.0 * 1.0)) / ((K * K) + 2.0 * 1.0 * 0.25 * K + (1.0 * 1.0));
    }
