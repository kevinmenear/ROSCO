...
#
#   bash evidence/PIController/run_stub.sh <stub.cpp> <log-name>
#
# Copied from evidence/saturate/run_stub.sh (unit #24) with two names changed
...
#     an unknown program;
#   * `rm -f picontroller.o kernel.exe` before `make -s build` -- a bare `make`
...
cd "$ROOT"
SRC="$1"; NAME="$2"
K=kernel/PIController

cp "$SRC" "$K/picontroller.hpp"
want=$(md5 -q "$SRC")
got=""
for i in 1 2 3; do
    got=$(docker exec vit-dev bash -lc "md5sum /workspace/ROSCO-r2/$K/picontroller.hpp | cut -d' ' -f1")
    [ "$want" = "$got" ] && break
    cp "$SRC" "$K/picontroller.hpp"
done
[ "$want" = "$got" ] || { echo "HASH MISMATCH: $want != $got"; exit 1; }
docker exec vit-dev bash -lc "touch /workspace/ROSCO-r2/$K/picontroller.hpp"

{
    ...
    docker exec vit-dev bash -lc \
        "cd /workspace/ROSCO-r2/$K && rm -f picontroller.o kernel.exe && make -s build && ./kernel.exe" 2>&1
} > "evidence/PIController/$NAME"
grep -E "verification-passed|Total number of verification cases|FAILED|PASSED" "evidence/PIController/$NAME" | head -5
echo "  IDENTICAL rows:     $(grep -c 'is IDENTICAL' "evidence/PIController/$NAME" || true)"
echo "  NOT IDENTICAL rows: $(grep -c 'NOT IDENTICAL' "evidence/PIController/$NAME" || true)"
