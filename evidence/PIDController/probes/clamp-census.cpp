// CENSUS PROBE, generated from the shipped translation by
// evidence/PIDController/make_census.py. It writes NO output the harness
// compares -- one CSV row per call to /workspace/ROSCO-r2/evidence/PIDController/clamp_census.csv
...
static void vit_census_open() {
    if (!vit_census) {
        vit_census = fopen("/workspace/ROSCO-r2/evidence/PIDController/clamp_census.csv", "w");
        if (vit_census)
...
double PIDController(double error, double kp, double ki, double kd, double tf, double minValue, double maxValue, double DT, double I0, piparams_t* piP, int32_t reset, objectinstances_t* objInst, localvariables_view_t* LocalVar) {
    ...
    double vit_raw_iterm_in = vit_nan, vit_raw_iterm_upd = vit_nan, vit_raw_sum = vit_nan;  // CENSUS
    ...
    if (reset != 0) {
        ...
        if (vit_census) {
            fprintf(vit_census,
                    "%d,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g\n",
                    (int)(reset != 0), minValue, maxValue, error, kp, ki, kd, tf, DT, I0,
                    EFilt, vit_nan, vit_nan, vit_nan, vit_nan, vit_nan, vit_nan, PIDController_result);
            fflush(vit_census);
        }
        ...
        vit_raw_iterm_upd = piP->ITerm[i];  // CENSUS
        ...
        if (vit_census) {
            fprintf(vit_census,
                    "%d,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g\n",
                    (int)(reset != 0), minValue, maxValue, error, kp, ki, kd, tf, DT, I0,
                    EFilt, PTerm, vit_raw_iterm_in, vit_raw_iterm_upd, piP->ITerm[i], DTerm, vit_raw_sum, PIDController_result);
            fflush(vit_census);
        }
