// PROBE -- NOT the translation. The shipped file with ONE inserted block:
// a determinate wrong answer returned whenever EITHER error input is
// non-zero. If this PASSES, both inputs are identically zero in every
// captured case and the kernel is testing this unit on all-zero data.
// The `!= 0.0` comparison is true for a NaN as well, which is what we want:
// the claim being tested is "the captured inputs are the literal zero".
...
double PIIController(double error, double error2, double kp, double ki, double ki2, double minValue, double maxValue, double DT, double I0, piparams_t* piP, int32_t reset, int* inst) {
    ...
    if (!(error == 0.0) || !(error2 == 0.0)) {  // PROBE
        (void)piP; (void)inst; return -7.25;
    }
    const int i = *inst - 1;
