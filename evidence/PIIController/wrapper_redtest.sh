...
#
#   bash evidence/PIIController/wrapper_redtest.sh <sed-expr> <out.json> "<what>"
#
...
#
# The sed is anchored on the PIIController wrapper's own line number range so it
...
# PIIController'` is itself ambiguous against `FUNCTION PIController` only
...
LO=$(grep -n 'FUNCTION PIIController' "$F" | head -1 | cut -d: -f1)
HI=$(grep -n 'END FUNCTION PIIController' "$F" | head -1 | cut -d: -f1)
...
bash scripts/harness.sh PIIController Controllers piicontroller "$F" \
