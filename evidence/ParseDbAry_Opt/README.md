...
**Disposition: `deferred`.** Five layers ran; four are green and red-tested and
the fifth, mutation, is **0.7681 against a threshold of 1.0**. That is this
...
| mutation (`mutation/ParseDbAry_Opt.json`) | **189 mutants, 4 nocompile, 185 behavioural — 106 KILLED, 17 EQUIVALENT, 30 UNREACHABLE, 32 SURVIVED, score 0.7681.** Sanitised sweep, green baseline, clean tree. `declared_but_killed` and `unreachable_but_killed` both empty | — the score IS the red test (E4.6). §5 |
...
| gate, 27 scenarios (`gate/ParseDbAry_Opt.json`) | 5,252,000 values / 351 channels, **0 mismatched** | **four**: every parsed value + 0.01 moves **2,236,141** (43%); the `Ary = 0` default arm **0**; survivor 48542e3d **0**; survivor ae3f319a **0**. §6 |
...
## 5. C6 — mutation: 0.7681, and the 32 that are left

```
189 mutants, 4 nocompile, 185 behavioural
106 KILLED   17 EQUIVALENT   30 UNREACHABLE   32 SURVIVED     0.7681
```
...
**The 32 that are left are named in `mutation_survivors.txt`**, by region and by
what each region's survival is a fact ABOUT: 9 in the PRINT record, which needs
a different INSTRUMENT; 7 in `match_word`, 5 in `list_read_reals` and 5
`p <= len` boundaries, which need a different CORPUS; 5 `rec[p + 1]` reads that
land on blank padding; and 1 each in `assign_errmsg` and the unit's own body.
...
* **Whatever the 32 surviving mutants would have caught**, and now also the 30
...
* **The `PRINT` record.** Translated and measured against the reference
  (`fortran_io_probe.txt`) but no layer *compares* it — 9 of the 32 survivors.
...
mutation_survivors.txt                   the 32 that are left, by region
