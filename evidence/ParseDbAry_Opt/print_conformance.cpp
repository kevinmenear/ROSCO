...
// enters `ParseDbAry_Opt`. If one is ever reached the stub aborts rather than
...
// The reference wrote `INTEGER` and `REAL(8)` with an unformatted STREAM
...
int main(int argc, char** argv) {
    ...
    for (;;) {
        ...
        std::vector<double> v(static_cast<std::size_t>(n < 0 ? 0 : n));
        for (int i = 0; i < n; ++i) {
