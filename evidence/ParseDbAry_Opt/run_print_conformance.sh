...
#
#   bash evidence/ParseDbAry_Opt/run_print_conformance.sh
#
# THIS IS A SECOND INSTRUMENT AND ITS RESULT IS FOLDED INTO NOTHING. A kill by a
# different instrument on a different corpus is not a kill by the sweep, and
# mutation/ParseDbAry_Opt.json is not touched by this script. What it buys is
# that "9 survivors nothing compares" becomes a measurement. Same rule unit #55
# recorded for `parser_conformance`.
#
...
TESTDIR=translations/ROSCO_Helpers/parsedbary_opt_test
...
def run(container_path, tag):
    ...
    cmd = (f"cd /workspace/ROSCO-r2/.print_conf && "
           ...
           f"/workspace/ROSCO-r2/evidence/ParseDbAry_Opt/print_conformance.cpp "
...
rc, got, err = run('/workspace/ROSCO-r2/translations/ROSCO_Helpers/parsedbary_opt.cpp',
                   'clean')
if rc != 0:
