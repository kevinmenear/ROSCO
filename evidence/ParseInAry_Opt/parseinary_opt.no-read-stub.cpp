...
void ParseInAry_Opt(char* FileLines, int n_FileLines, int len_FileLines,
    ...
    if (FoundLine) {
        ...
        // RED TEST -- the list-directed READ deleted. Predicted to fail 103 =
        // 61 (alloc=0) + 42 (alloc=1): every case whose reference ErrMsg
        // carries the parse-error arm. 103 of 13,674 is therefore the number of
        // cases that can distinguish ANYTHING about this parser -- 0.75% of the
        // corpus, and the single largest gap in this unit's evidence.
        ErrStatLcl = 0;
        (void)list_read_ints;
        for (int i = 0; i < n; ++i) {
