...
#
#   bash evidence/ParseInAry_Opt/run_harness_stub.sh <stub.cpp> <out.json> "<what was cut>"
#
# Copied from `evidence/ParseDbAry_Opt/run_harness_stub.sh` (P4), which copied
# it from `evidence/ChkParseData/run_harness_stub.sh`, with the unit's three
# names changed and nothing else. The shipped translation is restored on
...
docker exec vit-dev bash -lc \
    "cd /workspace/ROSCO-r2/translations/ROSCO_Helpers/parseinary_opt_test && \
     make test >/dev/null 2>&1 && ./test parseinary_opt_cases.bin" \
...
obj['red_test_note'] = ('run with --no-generate, so this is the same case file '
                        'the green in harness/ParseInAry_Opt.json was taken on')
