...
#
#   bash evidence/ParseInAry_Opt/run_partition_probe.sh
#
# COPIED from `evidence/ParseDbAry_Opt/run_partition_probe.sh` (P4) with the
# unit's two names changed and nothing else. The two units are the same
# subroutine with one declaration changed, and the classifier's message
# substrings are shared by both bodies.
#
# WHY IT IS A SCRIPT. This unit's first two dispatches produced
# `harness_partition.txt` with "one fprintf in the generated test and a
# --no-generate rebuild", and left no way to repeat it. The corpus has now moved
# 13,674 -> 15,504 -> 17,520 and the table -- which the wrapper red test's
# PREDICTED pass set is read out of -- has to be re-taken with it. A probe whose
# method is a sentence is a probe that expires with the corpus it was run on.
#
...
docker exec vit-dev bash -lc \
    "cd /workspace/ROSCO-r2/$D && rm -f parseinary_opt_test.o test && \
     make test > /dev/null 2>&1 && ./test parseinary_opt_cases.bin > /dev/null" \
    2> /tmp/parseinary_partition_raw.txt || true

python3 - /tmp/parseinary_partition_raw.txt <<'PY'
