...
#
#   bash evidence/ParseInAry_Opt/run_survivor_record_search.sh
#
...
# `evidence/ParseInAry_Opt/mutation_survivors.txt` rather than measured here.
#
...
cd "$ROOT"
OUT=evidence/ParseInAry_Opt/survivor_record_search.txt
...
# `evidence/ParseInAry_Opt/survivor_record_search.MISLABELLED.txt`.
def build(container_path, tag):
    cmd = (f"cd /workspace/ROSCO-r2/.record_search && "
           ...
           f"/workspace/ROSCO-r2/evidence/ParseInAry_Opt/survivor_record_search.cpp "
...
status, base, err = run('/workspace/ROSCO-r2/translations/ROSCO_Helpers/parseinary_opt.cpp',
                        'base')
report = []
if status != 'ok':
...
report.append("ParseInAry_Opt -- a record that distinguishes each surviving mutant")
...
report.append("those flags, which is the control that makes a mutant's abort its own.")
report.append("")
...
report.append("  is the record's last byte; plus 18 crafted full-width records. Three")
report.append("  elements are read, each pre-set to the -987654 sentinel, so an element")
