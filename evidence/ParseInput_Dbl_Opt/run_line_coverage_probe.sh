#!/bin/bash
# Which LINES of the shipped translation does the differential-harness corpus execute?
#
#   bash evidence/ParseInput_Dbl_Opt/run_line_coverage_probe.sh      # tree must be CLEAN
#
# COPIED CHARACTER FOR CHARACTER from
# `evidence/ParseInAry_Opt/run_line_coverage_probe.sh` (P4) -- itself a copy of
# `evidence/ParseDbAry_Opt`'s -- with the unit's two names changed and the corpus
# count now READ from the harness artifact rather than written into the prose.
#
# WHY. 67 mutants survived this unit's FIRST sweep. "Survived" has three causes and the artifact
...
#
# HOW. `vit test-validate` writes the translation to `parseinput_dbl_opt.hpp` and the
...
cd "$ROOT"
D=translations/ROSCO_Helpers/parseinput_dbl_opt_test
OUT=evidence/ParseInput_Dbl_Opt/line_coverage.txt
...
# \`parseinput_dbl_opt_callees.f90\` per tree (bridges kept on a clean tree, dropped on
...
# \`parseinput_dbl_opt.hpp\` IS ASKED OF MAKE, and that is not decoration. It is a
# COPY of the translation (Makefile:88-89, \`cp \$< \$@\`) and the probe compiles
# \`parseinput_dbl_opt_test.cpp\` with g++ DIRECTLY, so without naming it here the one
...
rm -f parseinput_dbl_opt.hpp
make parseinput_dbl_opt.hpp parseinput_dbl_opt_callees.o parseinput_dbl_opt_bridge.o
g++ -O0 -g -fPIC -ffp-contract=off -I. --coverage -c parseinput_dbl_opt_test.cpp -o cov_test.o
g++ --coverage -o cov_test cov_test.o parseinput_dbl_opt_bridge.o parseinput_dbl_opt_callees.o \$LIBS
./cov_test parseinput_dbl_opt_cases.bin > /dev/null 2>&1 || true
gcov -o . cov_test.o parseinput_dbl_opt_test.cpp > /dev/null 2>&1 || true
ls *.gcov
"

cp "$ROOT/$D/parseinput_dbl_opt.hpp.gcov" /tmp/parseinput_dbl_opt.hpp.gcov

python3 - /tmp/parseinput_dbl_opt.hpp.gcov "$OUT" <<'PY'
...
_checked = _json.loads(pathlib.Path(
    "harness/ParseInput_Dbl_Opt.json").read_text())["checked"]
body = [f"ParseInput_Dbl_Opt -- line coverage of the SHIPPED translation under the",
        f"{_checked:,}-case differential harness corpus, measured by gcov over the",
        f"generated test's own object (the test #includes parseinput_dbl_opt.hpp).",
        f"",
...
for ln, cnt, text in lines:
    if "void ParseInput_Dbl_Opt(" in text or "ParseInput_Dbl_Opt(char*" in text:
