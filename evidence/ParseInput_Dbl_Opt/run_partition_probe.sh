...
#
#   bash evidence/ParseInput_Dbl_Opt/run_partition_probe.sh
#
...
# which `parseinput_dbl_opt_f90` reaches real Fortran all the way down.
#
# The five arms, read off `ROSCO_Helpers.f90` at 54dd134 (lines 225-267):
#
...
T=$D/parseinput_dbl_opt_test.cpp
OUT=evidence/ParseInput_Dbl_Opt/harness_partition.txt
...
s = s.replace(snap,
              ...
              "        const double vit_sup_var = Variable_a;\n" + snap)
...
docker exec vit-dev bash -lc \
    "cd /workspace/ROSCO-r2/$D && rm -f parseinput_dbl_opt_test.o test && \
     make test > /dev/null 2>&1 && ./test parseinput_dbl_opt_cases.bin > /dev/null" \
    2> /tmp/parseinput_dbl_partition_raw.txt || true

python3 - /tmp/parseinput_dbl_partition_raw.txt "$OUT" <<'PY'
...
out.append(f"harness partition -- {len(rows)} case(s), "
           f"corpus translations/ROSCO_Helpers/parseinput_dbl_opt_test/"
