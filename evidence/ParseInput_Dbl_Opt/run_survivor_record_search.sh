...
#
#   bash evidence/ParseInput_Dbl_Opt/run_survivor_record_search.sh
#
...
# drives `list_read_reals`, which the PRINT record is not reached from, so a
...
# `evidence/ParseInput_Dbl_Opt/mutation_survivors.txt` rather than measured here.
#
...
cd "$ROOT"
OUT=evidence/ParseInput_Dbl_Opt/survivor_record_search.txt
...
surv = [s['id'] for s in
        json.load(open(os.path.join(root, 'mutation/ParseInput_Dbl_Opt.json')))['survivors']]
...
# `evidence/ParseInput_Dbl_Opt/survivor_record_search.MISLABELLED.txt`.
...
status, base, err = run('/workspace/ROSCO-r2/translations/ROSCO_Helpers/parseinput_dbl_opt.cpp',
                        'base')
report = []
if status != 'ok':
...
report.append("ParseInput_Dbl_Opt -- a record that distinguishes each surviving mutant")
