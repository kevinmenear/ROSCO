...
#
#   bash evidence/ParseInput_Dbl_Opt/run_wrapper_redtest.sh
#
...
# `evidence/ParseInput_Dbl_Opt/harness_partition.txt` rather than typed here:
...
#
#   not-allowed                 1845
#   error, msg refused           211
#                        ------------
#   PREDICTED                   2,056 must FAIL
#   the rest                    9,506 write no scalar and must PASS
#                        ------------
#                              11,562
#
# The two arms that write no scalar are `default-warned` (9,148 cases -- it
# writes `Variable` and a stdout record, and neither crosses the view struct)
# and `read-ok` (174). `pre-failed` (184) writes nothing at all.
#
...
start = next(i for i, l in enumerate(lines)
             if re.match(r"\s*SUBROUTINE ParseInput_Dbl_Opt\(", l))
end = next(i for i, l in enumerate(lines[start:], start)
           if re.match(r"\s*END SUBROUTINE ParseInput_Dbl_Opt\s*$", l))
inside = sum(lines[i].count(FROM) for i in range(start, end + 1))
print(f"  inside SUBROUTINE ParseInput_Dbl_Opt (lines {start+1}..{end+1}): {inside}")
...
bash scripts/harness.sh ParseInput_Dbl_Opt ROSCO_Helpers parseinput_dbl_opt "$F" --post-integration \
     --out harness/ParseInput_Dbl_Opt.postintegration.redtest.json \
...
bash scripts/harness.sh ParseInput_Dbl_Opt ROSCO_Helpers parseinput_dbl_opt "$F" --post-integration \
     --out harness/ParseInput_Dbl_Opt.postintegration.revert-verified.json 2>&1 | tail -3
