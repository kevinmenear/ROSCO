#!/usr/bin/env python3
"""Re-derive `mutation/ParseInput_Int_Opt.unreachable.json` from the MEASURED line
coverage, and refuse if the measurement is not of this corpus.

    python3 evidence/ParseInput_Int_Opt/make_unreachable.py
...
COV = ROOT / "evidence/ParseInput_Int_Opt/line_coverage.txt"
CPP = ROOT / "translations/ROSCO_Helpers/parseinput_int_opt.cpp"
HARNESS = ROOT / "harness/ParseInput_Int_Opt.json"
OUT = ROOT / "mutation/ParseInput_Int_Opt.unreachable.json"
...
def main() -> int:
    ...
    if f"{checked:,}-case" not in text:
        print(f"REFUSING: {COV} does not name a {checked:,}-case corpus -- "
              f"re-run evidence/ParseInput_Int_Opt/run_line_coverage_probe.sh",
              file=sys.stderr)
        return 2
    ctrl = re.search(r"L\d+\s+count=(\d+)\s+void ParseInput_Int_Opt", text)
    ...
    # `mutation/ParseInput_Int_Opt.equivalences.md` as one nonzero IOSTAT for
    # another under a single `!= 0` reader.
    equiv = set(json.loads((ROOT / "mutation/ParseInput_Int_Opt.equivalences.json"
                            ).read_text()))
    ...
    # `evidence/ParseInput_Int_Opt/make_nocompile_ids.py` writes by asking the
    ...
    NOCOMPILE = set(json.loads(
        (ROOT / "evidence/ParseInput_Int_Opt/nocompile_ids.json").read_text()))
    ...
    # `evidence/ParseInput_Int_Opt/remap_declarations.py` keys a declaration.
    CONTINUATION: dict[str, str] = {}

    src = CPP.read_text()
    out: dict[str, dict[str, str]] = {}
    skipped_equiv, skipped_nocompile = [], []
    for m in mutants("parseinput_int_opt", src):
        if m.mid in CONTINUATION and m.mid not in equiv:
            out[m.mid] = {"reason": CONTINUATION[m.mid],
                          "evidence": "evidence/ParseInput_Int_Opt/line_coverage.txt"}
            continue
        ...
        out[m.mid] = {
            "reason": (
                f"parseinput_int_opt.cpp:{m.line} is NEVER EXECUTED by any of the "
