...
#
#   bash evidence/ParseInput_Int_Opt/run_harness_stub.sh <stub.cpp> <out.json> "<what was cut>"
#
# Copied from `evidence/ParseInput_Int_Opt/run_harness_stub.sh`'s ancestors
...
cd "$ROOT"
STUB="$1"; OUT="$2"; WHAT="$3"
LIVE=translations/ROSCO_Helpers/parseinput_int_opt.cpp
KEEP=$(mktemp)
cp "$LIVE" "$KEEP"
...
bash scripts/harness.sh ParseInput_Int_Opt ROSCO_Helpers parseinput_int_opt \
     rosco/controller/src/ROSCO_Helpers.f90 \
     --no-generate --out "$OUT" --red-test "$WHAT" 2>&1 | tail -6

docker exec vit-dev bash -lc \
    "cd /workspace/ROSCO-r2/translations/ROSCO_Helpers/parseinput_int_opt_test && \
     make test >/dev/null 2>&1 && ./test parseinput_int_opt_cases.bin" \
...
obj['red_test_note'] = ('run with --no-generate, so this is the same case file '
                        'the green in harness/ParseInput_Int_Opt.json was taken on')
