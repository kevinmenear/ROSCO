...
#
#   bash evidence/ParseInput_Int_Opt/run_line_coverage_probe.sh      # tree must be CLEAN
#
...
#
# HOW. `vit test-validate` writes the translation to `parseinput_int_opt.hpp` and the
...
OUT=evidence/ParseInput_Int_Opt/line_coverage.txt
...
# \`parseinput_int_opt_callees.f90\` per tree (bridges kept on a clean tree, dropped on
...
# \`parseinput_int_opt.hpp\` IS ASKED OF MAKE, and that is not decoration. It is a
# COPY of the translation (Makefile:88-89, \`cp \$< \$@\`) and the probe compiles
# \`parseinput_int_opt_test.cpp\` with g++ DIRECTLY, so without naming it here the one
...
rm -f parseinput_int_opt.hpp
make parseinput_int_opt.hpp parseinput_int_opt_callees.o parseinput_int_opt_bridge.o
g++ -O0 -g -fPIC -ffp-contract=off -I. --coverage -c parseinput_int_opt_test.cpp -o cov_test.o
g++ --coverage -o cov_test cov_test.o parseinput_int_opt_bridge.o parseinput_int_opt_callees.o \$LIBS
./cov_test parseinput_int_opt_cases.bin > /dev/null 2>&1 || true
gcov -o . cov_test.o parseinput_int_opt_test.cpp > /dev/null 2>&1 || true
ls *.gcov
"

cp "$ROOT/$D/parseinput_int_opt.hpp.gcov" /tmp/parseinput_int_opt.hpp.gcov

python3 - /tmp/parseinput_int_opt.hpp.gcov "$OUT" <<'PY'
...
_checked = _json.loads(pathlib.Path(
    "harness/ParseInput_Int_Opt.json").read_text())["checked"]
body = [f"ParseInput_Int_Opt -- line coverage of the SHIPPED translation under the",
        f"{_checked:,}-case differential harness corpus, measured by gcov over the",
        f"generated test's own object (the test #includes parseinput_int_opt.hpp).",
        f"",
...
for ln, cnt, text in lines:
    if "void ParseInput_Int_Opt(" in text or "ParseInput_Int_Opt(char*" in text:
