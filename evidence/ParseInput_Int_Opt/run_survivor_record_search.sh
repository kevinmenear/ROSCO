...
report.append("")
report.append("THE LIST IT IS RUN OVER IS THE UNDECLARED SWEEP'S -- all 54 survivors of")
report.append("evidence/ParseInput_Int_Opt/sweep_probe0.json, before any equivalence or")
report.append("unreachable declaration was made -- and NOT the two that are still open in")
report.append("mutation/ParseInput_Int_Opt.json. That is deliberate: 37 of those 54 are now")
report.append("DECLARED EQUIVALENT, and the rows below are the measurement each of those")
report.append("declarations rests on. A declaration whose evidence file did not cover it")
report.append("would be an argument with nothing under it.")
report.append("")
report.append("SEVEN of the 54 are KILLED in the scored artifact, in two rounds, and their")
report.append("rows below are the reason: each named an ADMISSIBLE record the corpus did")
report.append("not contain, R14 gained it, and the sweep then killed it.")
report.append("")
report.append("  round 1, loop 343f843   08ce3b56  17de888e  9e4b76b0  cb35366f")
report.append("           four named, four killed, and nothing else moved: 89 -> 93")
report.append("  round 2, loop 581925d   3545d5d7  5002a20d  (ab7420d6 named and NOT")
report.append("           killed -- the form written for it could not reach the bound it")
report.append("           was aimed at, see mutation_survivors.txt) : 93 -> 95")
report.append("  round 3, loop 8f9d72f   ab7420d6, on the corrected form         : 95 -> 96")
report.append("")
report.append("  round 4, loop b3ad414   4f89b00b and e8cf2d25, the last two, on the four")
report.append("           end-of-line entries -- 96 -> 98, and the score is 1.000")
report.append("")
report.append("THE SEARCH SPACE GAINED THE BYTE THOSE TWO TURNED ON. The first dispatch")
report.append("closed saying `the search\'s own alphabet has no CR or LF either, and that is")
report.append("stated rather than hidden`, which made all 37 declared equivalences claims")
report.append("over a space that could not contain the one character the two open survivors")
report.append("distinguished. The EOLADM block closes it: it is APPENDED after every")
report.append("existing block, so every row above is byte-identical and only a mutant that")
report.append("NOTHING else distinguishes can take a row from it.")
...
report.append("")
report.append(f"RECORD SPACE: {len(baselines)} records, IN THREE PARTS, and the split is the")
report.append("point rather than a decoration -- unit #56 twice read a `differs` on a")
report.append("record `GetWords` cannot produce as a corpus lever.")
report.append("")
report.append("  ADMISSIBLE TO THE UNIT (tags HEAD, FULLADM, WORDADM). `GetWords` splits")
report.append("  on ' ,!;''\" and TAB and copies the word LEFT-JUSTIFIED into a")
report.append("  blank-padded 200-byte element, so every record of this unit is")
report.append("  <word><blanks> with no interior blank, no comma and no semicolon")
report.append("  anywhere. Every string of length 1..3 over")
report.append("  \"0123456789+-.eEdDqQnNaAiIfFtTyYxX/*\" at the HEAD, plus 23 crafted")
report.append("  full-width 200-byte words and 29 named short words.")
report.append("")
report.append("  FUNCTION-ONLY (tag FUNCONLY). Right-justified tokens, and tokens")
report.append("  containing a blank, a comma or a semicolon. A difference here is a fact")
report.append("  about `list_read_ints` and NOT a corpus lever, because no input to")
report.append("  ParseInput_Int_Opt can put such a record in Words(1).")
report.append("")
report.append("  ADMISSIBLE, CARRYING AN END-OF-LINE (tag EOLADM). Added at the second")
report.append("  dispatch and APPENDED after both blocks above, so no existing row")
report.append("  moves. CR and LF are not GetWords separators, so a run containing one")
report.append("  is copied into Words(1) whole and every record here is admissible")
report.append("  whatever its position in the file. One and two alphabet characters")
report.append("  with the byte before, between and after; the record that is ALL")
report.append("  end-of-line, which is the one shape that makes the LEADING scan reach")
report.append("  the record\'s last byte and which no block above can produce (a record")
report.append("  of all blanks is not a word); and the two bytes together, a DOS line")
report.append("  ending. The token is spelled `cr` or `lf` where the record holds the")
report.append("  byte, so the row format is unchanged.")
report.append("")
report.append("ONE element is read -- the reference transfers one -- pre-set to the")
report.append("-987654 sentinel and printed as a decimal, so an UNTOUCHED slot is")
report.append("distinguishable from a stored value. That matters more here than on the")
report.append("REAL sibling: the INTEGER reader stores NOTHING on any failure. The record")
report.append("lives in the first 200 bytes of a 400-byte object whose second half holds a")
report.append("parameter name, which is what Words(1) and Words(2) are.")
report.append("")
