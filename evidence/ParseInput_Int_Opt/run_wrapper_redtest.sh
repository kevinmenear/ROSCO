...
#
#   bash evidence/ParseInput_Int_Opt/run_wrapper_redtest.sh
#
...
# `evidence/ParseInput_Int_Opt/harness_partition.txt` rather than typed here:
...
start = next(i for i, l in enumerate(lines)
             if re.match(r"\s*SUBROUTINE ParseInput_Int_Opt\(", l))
end = next(i for i, l in enumerate(lines[start:], start)
           if re.match(r"\s*END SUBROUTINE ParseInput_Int_Opt\s*$", l))
inside = sum(lines[i].count(FROM) for i in range(start, end + 1))
print(f"  inside SUBROUTINE ParseInput_Int_Opt (lines {start+1}..{end+1}): {inside}")
...
bash scripts/harness.sh ParseInput_Int_Opt ROSCO_Helpers parseinput_int_opt "$F" --post-integration \
     --out harness/ParseInput_Int_Opt.postintegration.redtest.json \
...
bash scripts/harness.sh ParseInput_Int_Opt ROSCO_Helpers parseinput_int_opt "$F" --post-integration \
     --out harness/ParseInput_Int_Opt.postintegration.revert-verified.json 2>&1 | tail -3
