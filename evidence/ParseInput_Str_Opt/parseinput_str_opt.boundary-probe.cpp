...
void ParseInput_Str_Opt(char* FileLines, int n_FileLines, int len_FileLines,
    ...
    // ---------------------------------------------------------------------
    // BOUNDARY PROBE -- not part of the translation. This file is a VARIANT of
    // the shipped one, used once, to answer two surviving mutants by MEASURING
    // what they change AT THEIR SITE rather than arguing about the corpus at
    // one remove.
    //
    //   4d525138  MaxLineLength  2048 -> 2049
    //   739190c8  MaxParamLength  200 -> 201
    //
    // Both constants reach exactly one live consumer, `getwords_c`, plus a
    // buffer size and a `ftrim` in the DEAD error arm. So the whole question is
    // whether `GetWords` returns different words when it is told the line is
    // one byte longer, or asked to fill elements one byte wider -- and that is
    // one extra call each, per case, next to the real one.
    //
    // The mutant's `Line` is a 2049-byte vector: findline_c writes exactly 2048
    // bytes (its own `CHARACTER(MaxLineLength)` dummy, no length crosses), so
    // byte 2049 is the vector's own zero. Reproduced here rather than assumed.
    {
        std::vector<char> Line2049(static_cast<std::size_t>(MaxLineLength) + 1, 0);
        std::memcpy(Line2049.data(), Line.data(),
                    static_cast<std::size_t>(MaxLineLength));
        std::vector<char> W2049(static_cast<std::size_t>(2) *
                                static_cast<std::size_t>(MaxParamLength));
        getwords_c(Line2049.data(), MaxLineLength + 1, W2049.data(),
                   MaxParamLength, 2);

        std::vector<char> W201(static_cast<std::size_t>(2) *
                               static_cast<std::size_t>(MaxParamLength + 1));
        getwords_c(Line.data(), MaxLineLength, W201.data(),
                   MaxParamLength + 1, 2);

        std::fprintf(stderr, "BND lastline=%d w1_2049=%d w2_2049=%d "
                             "w1_201=%d byte201=%d lenvar=%d\n",
                     static_cast<int>(static_cast<unsigned char>(
                         Line[MaxLineLength - 1])),
                     static_cast<int>(std::memcmp(W2049.data(), Words.data(),
                                                  MaxParamLength) != 0),
                     static_cast<int>(std::memcmp(
                         W2049.data() + MaxParamLength,
                         Words.data() + MaxParamLength, MaxParamLength) != 0),
                     static_cast<int>(std::memcmp(W201.data(), Words.data(),
                                                  MaxParamLength) != 0),
                     static_cast<int>(static_cast<unsigned char>(
                         W201[MaxParamLength])),
                     len_Variable);
