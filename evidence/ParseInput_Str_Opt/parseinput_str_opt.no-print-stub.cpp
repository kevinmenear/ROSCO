...
void ParseInput_Str_Opt(char* FileLines, int n_FileLines, int len_FileLines,
    ...
    if (!FoundLine) {
        ...
        // `Variable = 'unused'` on the line above, is untouched -- so the ONLY
        // output this stub can move is the record, which is the cell
        // `vit_record = { compare_record = ... }` gave the harness an oracle
        // in. Without that entry this stub moves ZERO.
