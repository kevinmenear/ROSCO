...
#
#   bash evidence/ParseInput_Str_Opt/run_harness_stub.sh <stub.cpp> <out.json> "<what was cut>"
#
# Copied from `evidence/ParseInput_Str_Opt/run_harness_stub.sh`'s ancestors
...
bash scripts/harness.sh ParseInput_Str_Opt ROSCO_Helpers parseinput_str_opt \
     rosco/controller/src/ROSCO_Helpers.f90 \
     --no-generate --out "$OUT" --red-test "$WHAT" 2>&1 | tail -6

docker exec vit-dev bash -lc \
    "cd /workspace/ROSCO-r2/translations/ROSCO_Helpers/parseinput_str_opt_test && \
     make test >/dev/null 2>&1 && ./test parseinput_str_opt_cases.bin" \
...
obj['red_test_note'] = ('run with --no-generate, so this is the same case file '
                        'the green in harness/ParseInput_Str_Opt.json was taken on')
