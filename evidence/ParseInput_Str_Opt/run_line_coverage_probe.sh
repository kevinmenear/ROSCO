...
#
#   bash evidence/ParseInput_Str_Opt/run_line_coverage_probe.sh      # tree must be CLEAN
#
...
#
# HOW. `vit test-validate` writes the translation to `parseinput_str_opt.hpp` and the
...
OUT=evidence/ParseInput_Str_Opt/line_coverage.txt
...
# \`parseinput_str_opt_callees.f90\` per tree (bridges kept on a clean tree, dropped on
...
# \`parseinput_str_opt.hpp\` IS ASKED OF MAKE, and that is not decoration. It is a
# COPY of the translation (Makefile:88-89, \`cp \$< \$@\`) and the probe compiles
# \`parseinput_str_opt_test.cpp\` with g++ DIRECTLY, so without naming it here the one
...
rm -f parseinput_str_opt.hpp
make parseinput_str_opt.hpp parseinput_str_opt_callees.o parseinput_str_opt_bridge.o
g++ -O0 -g -fPIC -ffp-contract=off -I. --coverage -c parseinput_str_opt_test.cpp -o cov_test.o
g++ --coverage -o cov_test cov_test.o parseinput_str_opt_bridge.o parseinput_str_opt_callees.o \$LIBS
./cov_test parseinput_str_opt_cases.bin > /dev/null 2>&1 || true
gcov -o . cov_test.o parseinput_str_opt_test.cpp > /dev/null 2>&1 || true
ls *.gcov
"

cp "$ROOT/$D/parseinput_str_opt.hpp.gcov" /tmp/parseinput_str_opt.hpp.gcov

python3 - /tmp/parseinput_str_opt.hpp.gcov "$OUT" <<'PY'
...
_checked = _json.loads(pathlib.Path(
    "harness/ParseInput_Str_Opt.json").read_text())["checked"]
body = [f"ParseInput_Str_Opt -- line coverage of the SHIPPED translation under the",
        f"{_checked:,}-case differential harness corpus, measured by gcov over the",
        f"generated test's own object (the test #includes parseinput_str_opt.hpp).",
        f"",
...
for ln, cnt, text in lines:
    if "void ParseInput_Str_Opt(" in text or "ParseInput_Str_Opt(char*" in text:
