// STUB, not a translation. The real body with the OpenLoop arm's three
// `interp1d` calls replaced by the `1.0` its ELSE alternatives assign -- the
// exact defect the corpus BEFORE the R11 baseline could not see, since it
// entered that arm's indexed branches zero times.
#include "vit_types.h"
