// PROBE, not a translation. The real body with ONE character changed: the C++
// side writes its list-directed records to `fort.401.cpp` instead of to
// `fort.401`, so the Fortran reference's own `fort.401` and the translation's
// copy end up in two files that can be diffed.
//
// WHY IT IS NEEDED AT ALL. The reference and the translation run in ONE process
// in this harness, and both write unit 401 -- to the same path, at independent
// offsets. That is not a comparison: it is two writers landing on one file, and
// a byte that agrees agrees by coincidence of ordering. Splitting the path is
// the whole probe.
//
// This is the ONLY instrument in the campaign that compares this output. The
// kernel cannot (the arm is dead in all 27 scenarios), the gate cannot (it
// compares avrSWAP channels), and the differential harness cannot (it compares
// the mapped signature, and no signature carries a file).
#include "vit_types.h"
...
void write_401(double v) {
    ...
    if (unit401 == nullptr) {
        unit401 = std::fopen("fort.401.cpp", "w");
        if (unit401 == nullptr) {
            return;
        }
    }
