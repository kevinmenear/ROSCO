...
#
#   bash evidence/PowerControlSetpoints/wrapper_redtest.sh <sed-expr> <out.json> "<what>"
#
...
#
# The sed is anchored on the PowerControlSetpoints wrapper's own line number range so it
...
# PowerControlSetpoints'` is itself ambiguous against `FUNCTION PIController` only
...
LO=$(grep -n 'SUBROUTINE PowerControlSetpoints' "$F" | head -1 | cut -d: -f1)
HI=$(grep -n 'END SUBROUTINE PowerControlSetpoints' "$F" | head -1 | cut -d: -f1)
...
bash scripts/harness.sh PowerControlSetpoints ControllerBlocks powercontrolsetpoints "$F" \
