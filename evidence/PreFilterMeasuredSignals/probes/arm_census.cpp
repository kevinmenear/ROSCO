// COUNTING PROBE, not a translation. The shipped body with one counter per arm
// and a destructor that writes them out. It answers ONE question the harness's
// own PASS cannot: WHICH arms do the 9033 generated cases run? A green is a
// statement about the arms its cases reach and nothing else, and this unit's
// no-op red test says only 293 of the 9033 get past the aviFAIL guard at all.
//
//   bash evidence/PreFilterMeasuredSignals/run_probe.sh \
//        evidence/PreFilterMeasuredSignals/probes/arm_census.cpp \
//        evidence/PreFilterMeasuredSignals/probes/arm_census.json "arm census"
//
// It changes no compared output, so the run that produces it is GREEN -- a
// READING of the corpus rather than a perturbation of it. Every counter
// increments on the C++ side only; the Fortran reference takes the same path on
// the same case by construction, since the harness hands both sides one input.
#include <cstdio>
namespace probe {
long n_call = 0, n_ret = 0, n_lpf1 = 0, n_lpf2 = 0, n_lpf_none = 0,
     n_genspd_iter = 0, n_zero_branch = 0, n_twrtop_iter = 0, n_td = 0,
     n_blade_iter = 0, n_ipc_arm = 0, n_flp2_arm = 0, n_else_arm = 0,
     n_flp2_notch_iter = 0;
struct Dump {
    ~Dump() {
        std::FILE* f = std::fopen(
            "/workspace/ROSCO-r2/evidence/PreFilterMeasuredSignals/probes/arm_census.txt", "w");
        if (!f) return;
        std::fprintf(f, "calls                                  %ld\n", n_call);
        std::fprintf(f, "  RETURNed on aviFAIL < 0              %ld\n", n_ret);
        std::fprintf(f, "  ran the body                         %ld\n", n_call - n_ret);
        std::fprintf(f, "F_LPFType == 1  (LPFilter)             %ld\n", n_lpf1);
        std::fprintf(f, "F_LPFType == 2  (SecLPFilter)          %ld\n", n_lpf2);
        std::fprintf(f, "F_LPFType matched NEITHER              %ld\n", n_lpf_none);
        std::fprintf(f, "gen-speed notch loop, iterations       %ld\n", n_genspd_iter);
        std::fprintf(f, "iStatus == 0 .AND. Time == 0           %ld\n", n_zero_branch);
        std::fprintf(f, "tower-top notch loop, iterations       %ld\n", n_twrtop_iter);
        std::fprintf(f, "TD_Mode > 0                            %ld\n", n_td);
        std::fprintf(f, "blade loop, iterations                 %ld\n", n_blade_iter);
        std::fprintf(f, "  IPC_ControlMode>0 .OR. Flp_Mode==3   %ld\n", n_ipc_arm);
        std::fprintf(f, "  Flp_Mode == 2                        %ld\n", n_flp2_arm);
        std::fprintf(f, "    its inner notch loop, iterations   %ld\n", n_flp2_notch_iter);
...
void PreFilterMeasuredSignals(controlparameters_view_t* CntrPar, localvariables_view_t* LocalVar,
    ...
    if (CntrPar->F_LPFType != 1 && CntrPar->F_LPFType != 2) ++probe::n_lpf_none;
