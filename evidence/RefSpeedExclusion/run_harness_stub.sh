...
#
#   bash evidence/RefSpeedExclusion/run_harness_stub.sh <stub.cpp> <out.json> "<what was cut>"
#
# Copied from `evidence/ParseInput_Str_Opt/run_harness_stub.sh` (P4), whose own
# ancestry is evidence/ParseInAry_Opt -> evidence/ParseDbAry_Opt ->
# evidence/ChkParseData, with the unit's three names changed and nothing else.
# The shipped translation is restored on
...
# `ControllerBlocks.f90.o` IS the wrapper -- there is no independent reference
# left to compare against.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"
STUB="$1"; OUT="$2"; WHAT="$3"
LIVE=translations/ControllerBlocks/refspeedexclusion.cpp
KEEP=$(mktemp)
cp "$LIVE" "$KEEP"
...
bash scripts/harness.sh RefSpeedExclusion ControllerBlocks refspeedexclusion \
     rosco/controller/src/ControllerBlocks.f90 \
     --no-generate --out "$OUT" --red-test "$WHAT" 2>&1 | tail -6

docker exec vit-dev bash -lc \
    "cd /workspace/ROSCO-r2/translations/ControllerBlocks/refspeedexclusion_test && \
     make test >/dev/null 2>&1 && ./test refspeedexclusion_cases.bin" \
...
obj['red_test_note'] = ('run with --no-generate, so this is the same case file '
                        'the green in harness/RefSpeedExclusion.json was taken on')
