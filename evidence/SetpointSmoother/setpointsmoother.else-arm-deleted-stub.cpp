...
void SetpointSmoother(localvariables_view_t* LocalVar, controlparameters_view_t* CntrPar, objectinstances_t* objInst) {
    ...
    if (CntrPar->SS_Mode == 1) {
        ...
        // ARM CENSUS probe B -- the ELSE arm's `SS_DelOmegaF = 0` deleted.
        // Fails exactly the cases that reach the ELSE arm AND arrive with a
        // non-zero SS_DelOmegaF, which is the only way that write is visible.
    }
}
