...
void Shutdown(localvariables_view_t* LocalVar, controlparameters_view_t* CntrPar,
    ...
    if ((LocalVar->SD_Trigger == 0) && (LocalVar->Time >= CntrPar->SD_TimeActivate)) {
        ...
        // RED TEST STUB 3 -- THE SD_EnableTime TRIGGER ARM DELETED.
        //
        // ASKS: is the ONE trigger condition that fires in scenario 9 visible to
        // the kernel? Coverage records ControllerBlocks.f90:683 at exactly 1 hit
        // and the other three trigger assignments at 0, and the field census
        // over the shipped run shows `sd_trigger` at 4 in 35 of the 62 cases.
        //
        // EXPECT: FAIL. If it passes, the 35 cases are carrying a trigger the
        // captured state already held rather than one this unit computes.
    }
