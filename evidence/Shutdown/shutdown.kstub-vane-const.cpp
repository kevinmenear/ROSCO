...
void Shutdown(localvariables_view_t* LocalVar, controlparameters_view_t* CntrPar,
    ...
    SD_NacVaneSinF = lpfilter_c(std::sin(LocalVar->NacVane * D2R), LocalVar->DT,
                                CntrPar->SD_YawErrorCornerFreq, &LocalVar->FP, LocalVar->iStatus,
                                LocalVar->restart ? 1 : 0, &objInst->instLPF, 0, 0.0);
    // RED TEST STUB 2 -- THE VANE ARITHMETIC REPLACED BY A CONSTANT.
    //
    // Both LPFilter calls are KEPT, so `objInst%instLPF` still advances by four
    // and the filter state arrays are still written in the same four slots:
    // the only thing this stub deletes is the atan2/R2D/wrap_180 combination
    // that turns them into `SD_NacVaneF`.
    //
    // ASKS: does the kernel's corpus contain any case where that combination is
    // not zero? The field census over the shipped run says `sd_nacvanef` is
    // 0.0000000000000000 in all 62 cases, so the expected answer is that this
    // stub PASSES -- which is not a defect in the stub but a named blindness in
    // the kernel, and the reason the differential harness exists.
    (void)SD_NacVaneSinF;
    (void)SD_NacVaneCosF;
    LocalVar->SD_NacVaneF = 0.0;
