// KERNEL RED-TEST STUB, not a translation. THE FREEWHEEL LOW-SPEED ARM DELETED.
//
// The shipped translation with ONE statement removed: the write
// `LocalVar%SU_LoadStageStartTime = LocalVar%Time` under
// `(SU_Stage == 1 .AND. SU_RotSpeedF < 0.95 * SU_RotorSpeedThresh)`.
//
// THIS STUB IS EXPECTED TO PASS 83 OF 83, and that is the measurement. It asks
// whether the kernel -- 83 cases drawn from the only scenario that calls this
// unit at all -- can see the one statement `coverage/line_coverage.json` puts
// at ZERO hits in all 27 scenarios. Scenario 9 starts the rotor at 4 rpm =
// 0.419 rad/s against a threshold of 0.95 * 0.3 = 0.285, so the test is false
// on every one of its 11,999 calls.
//
// A pass here is a third instrument agreeing with the other two:
// gate/Startup.redtest.freewheel_arm.json moves 0 of 5,252,000 on the same
// deletion, and the differential harness kills negate_cond on that condition on
// 5618 of 7151 cases. Executed by no simulation, reachable only by the harness.
...
void Startup(localvariables_view_t* LocalVar, controlparameters_view_t* CntrPar,
    ...
    if ((LocalVar->SU_Stage == 1) &&
        ...
        // RED TEST: the freewheel low-speed write deleted
    }
