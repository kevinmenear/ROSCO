#!/bin/bash
# ONE differential-harness red test per invocation, for unit #43
# `StructuralControl`.
#
#   bash scripts/run_if_time_remains.sh 300 \
#       bash evidence/StructuralControl/run_harness_redtest.sh <stub.cpp> <out.json> "<what was perturbed>"
#
...
# the corpus that proves the harness can fail. The generating run wrote 1227
...
E=evidence/StructuralControl
trap 'git checkout -- "$CPP"; echo "restored $CPP"' EXIT
cp "$E/$1" "$CPP"
bash scripts/harness.sh StructuralControl Controllers structuralcontrol \
