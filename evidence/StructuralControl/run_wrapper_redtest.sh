...
#
#   bash scripts/run_if_time_remains.sh 400 bash evidence/StructuralControl/run_wrapper_redtest.sh
#
...
# its principal output LocalVar%StC_Input is a FIXED-SIZE field the view carries
# by value and not a C_LOC'd allocatable.
#
# The perturbation is the deletion of exactly that line, scoped to THIS unit's
# wrapper by evidence/StructuralControl/scope_wrapper_perturb.py -- the line
# appears several times in Controllers.f90 and a whole-file replace measures
# several wrappers. Read that script's header; the scoping is unit #41's finding.
#
...
python3.12 evidence/StructuralControl/scope_wrapper_perturb.py "$F" || exit 2
touch "$F"; build
bash scripts/harness.sh StructuralControl Controllers structuralcontrol "$F" \
    --post-integration --out evidence/StructuralControl/harness.postintegration.redtest.json \
    --red-test "StructuralControl's wrapper: CALL vit_copy_scalars_to_localvariables deleted -- every write to LocalVar%StC_Input never leaves the view" 2>&1 | tail -8
