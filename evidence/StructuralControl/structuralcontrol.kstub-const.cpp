...
// VIT: 0.1.0
// Status: KERNEL RED-TEST STUB -- the first step constant moved. Not a translation.
//
// PREDICTED BEFORE IT WAS RUN: this stub fails EXACTLY 35 of the 62 cases -- the
// 14 in 20,002..20,015 and the 21 in 23,975..23,995, which are the invocations
// with `Time > 500`. The other 27 (1..20 and 19,995..20,001) are below the step
// and the arm does not run. This is the counterpart to the no-op: it says the
// kernel CAN see the one statement the corpus reaches, and how many cases of it.
...
void StructuralControl(float* avrSWAP, controlparameters_view_t* CntrPar,
    ...
    if (CntrPar->StC_Mode == 1) {
        ...
        if (LocalVar->Time > 500.0) {
            ...
            LocalVar->StC_Input[0] = -1.235e+06;  // KERNEL RED TEST
