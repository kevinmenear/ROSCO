...
// VIT: 0.1.0
// Status: KERNEL RED-TEST STUB -- the avrSWAP copy loop deleted. Not a translation.
//
// PREDICTED BEFORE IT WAS RUN: this stub passes 62 of 62. `avrswap` is not among
// the ~250 field names in kernel/StructuralControl/verify_fields.csv, so the
// kernel compares no part of the array this loop writes. The prediction is that
// deleting the ONLY statement in this unit that writes outside `LocalVar` is
// INVISIBLE to the kernel -- a named blindness, measured rather than asserted,
// and the reason the differential harness rather than the kernel is this unit's
// primary evidence. The gate DOES see it: gate/StructuralControl.redtest.step_constant.json
// moves 7,998 values through exactly this loop.
...
void StructuralControl(float* avrSWAP, controlparameters_view_t* CntrPar,
    ...
    // KERNEL RED TEST: the whole avrSWAP copy loop deleted.
