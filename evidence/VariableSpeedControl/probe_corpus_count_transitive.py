...
"""How many cases does this unit's corpus WANT, and which rule asks for them.
    ...
    docker exec vit-dev bash -lc "cd /workspace/ROSCO-r2 && \
        python3 evidence/VariableSpeedControl/probe_corpus_count_transitive.py"
...
case carries a value for all 481 C parameters, and the array-valued ones are
Python lists filled per case (`avrSWAP` alone is 3,000 elements). This
...
while the retained memory per case is ~200 bytes instead of ~188 KB.
