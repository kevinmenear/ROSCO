// KERNEL RED TEST STUB for unit #60 VariableSpeedControl -- the avrSWAP(47) write deleted
//
...
//
// The ONE line that produces this procedure's whole reason for existing --
// the demanded generator torque handed back to the simulator -- is removed and
// nothing else is touched.
//
// EXPECT: 60 of 60 PASS, UNMOVED. `avrswap` is not among the 419 field names in
// kernel/VariableSpeedControl/verify_fields.csv; VIT's own comment in the
// generated DISCON.F90 says `skipped avrSWAP verification (assumed-size
// incompatible with KGen comparator)`. Units #43 and #44 measured the same
// blindness on their own avrSWAP writes; this is the third instance and the
// first where the deleted value is the unit's entire output.
...
void VariableSpeedControl(float* avrSWAP, controlparameters_view_t* CntrPar,
    ...
    // KERNEL RED TEST STUB: the avrSWAP(47) write DELETED. See the header.
    (void)avrSWAP;
