...
struct Dump {
    ~Dump() {
        for (const Sub& s : subs)
            std::fprintf(stderr, "SUBCENSUS %-4s extent=%d min=%d max=%d uses=%ld\n",
...
void WindSpeedEstimator(localvariables_view_t* LocalVar, controlparameters_view_t* CntrPar,
    ...
    if (CntrPar->WE_Mode == 1 && LocalVar->WE_Op > 0) {
        ...
        if (LocalVar->RestartWSE == 0) {
            ...
            else if (Cp_op == 0.0) ++vitcensus::cp_zero;
            else ++vitcensus::cp_neg;
            if (LocalVar->WE.v_h == 0.0) ++vitcensus::vh_zero;
            if (!std::isfinite(LocalVar->WE.v_h)) ++vitcensus::vh_nonfinite;
            if (LocalVar->WE.om_r == 0.0) ++vitcensus::omr_zero;
            ...
            if (F[1 * 3 + 0] != 0.0) ++vitcensus::F12_nonzero;
