#!/bin/bash
# ONE kernel hand red test per invocation, for unit #44 `YawRateControl`.
#
# `vit verify` DECLINED to construct its own red test here and printed
# NON_DISCRIMINATING -- the same refusal it gave units #40, #41, #42 and #43,
# and for the same stated reason: every argument arrives behind a pointer or
# inside a derived type, so there is no by-value floating-point input it can
# perturb without changing what BOTH implementations compute. So the kernel's
# discriminating power has to be measured by hand or not at all.
#
# AND FOR THIS UNIT THERE IS A SECOND REASON TO MEASURE IT BY HAND. The kernel
# reports 102 of 104 with the C++ in place AND 102 of 104 with the reference's
# own Fortran body in place, failing the same two cases with the same numbers --
# see evidence/YawRateControl/kernel.save_state_control.txt. A layer that cannot
# score two of its own cases needs its remaining power stated as a number.
#
...
# takes ~230s at 104 cases.
#
#   bash scripts/run_if_time_remains.sh 400 \
#       bash evidence/YawRateControl/run_one_kernel_stub.sh <stub.cpp> "<name>" "<asks>" "<expect>"
#
...
# `yawratecontrol_c`, and the green these stubs are red tests FOR was taken
...
# unit #11):
#
#   git show 8a9c241a^:rosco/controller/src/Controllers.f90 \
...
E=evidence/YawRateControl; OUT=$E/kernel.stubs.txt
REF=.vit/Controllers.preint.f90
[ -s "$REF" ] || { echo "run_one_kernel_stub: $REF is missing; see this script's header" >&2; exit 2; }
trap 'git checkout -- "$CPP" vit.yaml; docker exec vit-dev bash -lc "pkill -9 -f \"vit verify YawRateControl\"" >/dev/null 2>&1; echo "restored $CPP and vit.yaml"' EXIT
{ echo "=================================================================="
  echo "STUB      $2"; echo "ASKS      $3"; echo "EXPECT    $4"; echo "RESULT:"; } >> "$OUT"
cp "$E/$1" "$CPP"
docker exec vit-dev bash -lc "cd /workspace/ROSCO-r2 && vit verify YawRateControl $CPP \
    -f $REF --kernel-dir kernel/YawRateControl" 2>&1 \
  | grep -E "VERIFICATION|NON_DISCRIMINATING|passed|FAILED|mismatch" | head -6 | sed 's/^/    /' >> "$OUT"
docker exec vit-dev bash -lc "cd /workspace/ROSCO-r2/kernel/YawRateControl && ./kernel.exe 2>&1 \
