// KERNEL RED-TEST STUB, not a translation. THE RIGHT STOP'S STATE WRITE DELETED.
//
// Asks: does this kernel's corpus EXECUTE the stop arms that the differential
// harness cannot reach?
//
// Eight of the eleven mutants that survive the 8093-case differential sweep sit
// inside this unit's two stop arms, and they survive for a reason measured in
// evidence/YawRateControl/arm_census.txt: entering a state arm at all needs
// `iStatus /= 0`, and at `iStatus /= 0` the harness hands LPFilter a
// zero-initialised `FP`, so the callee returns NaN and every comparison against
// it is false. `from 1: stop 0`, `from -1: stop 0`, on all 725 entries.
//
// The kernel does not have that problem: `FP` is captured from a running
// simulation, and vit.yaml's window was aimed at the four state-machine edges
// before any of this was known. So this stub is the measurement that the layer
// the harness is blind to is a layer some other instrument is not.
//
// One statement is deleted -- `YawState = 0` in the `YawState == 1` arm. The
// commanded rate is still zeroed there, so the immediate output is unchanged
// and only the STATE diverges, which is what makes the first failing case
// predictable.
#include "vit_types.h"
    ...
    if (CntrPar->Y_ControlMode == 1) {
        ...
        if (YawState == 1) {
            if (NacHeadingError <= 0.0) {
                // KERNEL RED TEST: `YawState = 0` deleted from the RIGHT stop.
                YawRateCom = 0.0;
            } else {
                // ! persist
                YawRateCom = CntrPar->Y_Rate;
