// CENSUS PROBE -- the shipped translation plus one fprintf to stderr.
// Changes no compared output.
...
double ratelimit(double inputSignal, double minRate, double maxRate, double DT, int32_t reset, rlparams_t* rlP, int* inst, int has_ResetValue, double ResetValue) {
    ...
    const int i = *inst - 1;
    const double lastsignal_on_entry = rlP->LastSignal[i];  // CENSUS PROBE only
    ...
    if (reset != 0) {
        ...
        ratelimit_result = rlP->LastSignal[i] + rate * DT;
    ...
    *inst = *inst + 1;
    ...
    // CENSUS PROBE -- writes one line per call to stderr and changes NO
    // compared output, so the run this produces is GREEN and the numbers are a
    // reading of the captured corpus rather than a perturbation of it. Same
    // shape as unit #34's clamp_census. `unclamped` is the rate before
    // saturate_c; the kernel calls the body three times per case (evalstage,
    // warmupstage, mainstage) so every case appears three times.
    {
        double unclamped = (inputSignal - lastsignal_on_entry) / DT;
        std::fprintf(stderr,
            "CENSUS inst=%d reset=%d has_rv=%d in=%.17g last_in=%.17g rv=%.17g "
            "DT=%.17g minRate=%.17g maxRate=%.17g raw=%.17g out=%.17g\n",
            *inst - 1, (int)reset, has_ResetValue, inputSignal,
            lastsignal_on_entry, ResetValue, DT, minRate, maxRate,
            unclamped, ratelimit_result);
    }
