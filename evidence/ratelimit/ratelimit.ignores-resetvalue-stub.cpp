// STUB -- NOT A TRANSLATION. The OPTIONAL ResetValue is ignored.
...
double ratelimit(double inputSignal, double minRate, double maxRate, double DT, int32_t reset, rlparams_t* rlP, int* inst, int has_ResetValue, double ResetValue) {
    ...
    // STUB: `IF (PRESENT(ResetValue)) ResetValue_ = ResetValue` DELETED, so the
    // reset arm restarts from `inputSignal` rather than from the caller's
    // ResetValue. Observable ONLY where `reset` is true.
    double ResetValue_ = inputSignal;
    (void)has_ResetValue; (void)ResetValue;
