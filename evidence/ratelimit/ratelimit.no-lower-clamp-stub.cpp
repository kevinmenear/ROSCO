// STUB -- NOT A TRANSLATION. The LOWER rate clamp deleted.
...
double ratelimit(double inputSignal, double minRate, double maxRate, double DT, int32_t reset, rlparams_t* rlP, int* inst, int has_ResetValue, double ResetValue) {
    ...
    if (reset != 0) {
        ...
        // STUB: the LOWER clamp deleted -- fmin(rate, maxRate) instead of
        // saturate_c(rate, minRate, maxRate). `saturate_c` is still named
        // below in dead code so the kernel's link set is unchanged (unit #45).
        rate = std::fmin(rate, maxRate);
        if (DT == -1234.5) rate = saturate_c(rate, minRate, maxRate);
