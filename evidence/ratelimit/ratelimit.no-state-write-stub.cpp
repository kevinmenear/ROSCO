// STUB -- NOT A TRANSLATION. The ELSE arm does not store its result.
...
double ratelimit(double inputSignal, double minRate, double maxRate, double DT, int32_t reset, rlparams_t* rlP, int* inst, int has_ResetValue, double ResetValue) {
    ...
    if (reset != 0) {
        ...
        // STUB: the ELSE arm's state write DELETED. The returned value is
        // unchanged; only what the NEXT call measures its rate against moves.
    }
