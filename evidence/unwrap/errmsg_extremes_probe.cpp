// PROBE, NOT THE SHIPPED TRANSLATION -- the shipped translation with COUNTERS
// and nothing else changed. It answers the two UNREACHABLE claims in
// mutation/unwrap.equivalences.json with numbers over this unit's own corpus
// rather than with an argument:
//
//   af5a7c94  `n > 0 ? static_cast<size_t>(n) : 0` -> `: 1`
//             reachable only if some call arrives with n_ErrMsg <= 0
//   10e6dfb3  `static_cast<int>(s.size()) > cap` -> `>=`
//             reachable only if some assignment is EXACTLY the capacity
//
// Run through the harness's own Makefile (evidence/unwrap/run_errmsg_probe.sh),
// which is why it is swapped over translations/Functions/unwrap.cpp: the
// generated Makefile's FIRST rule copies that file to unwrap.hpp.
...
#include <climits>
namespace vitprobe {
long calls = 0, assigns = 0;
int  n_min = INT_MAX, n_max = INT_MIN;
int  cap_min = INT_MAX, cap_max = INT_MIN;
int  biggest_msg = -1;
long n_nonpositive = 0, size_eq_cap = 0, size_gt_cap = 0, no_buffer = 0;
void report() {
    std::fprintf(stderr,
        "calls to unwrap                        %ld\n"
        "ErrVar->n_ErrMsg on entry, min .. max  %d .. %d\n"
        "calls with n_ErrMsg <= 0               %ld     <- af5a7c94 needs > 0\n"
        "ErrVar->n_ErrMsg_cap, min .. max       %d .. %d\n"
        "calls with ErrMsg == nullptr           %ld\n"
        "calls to assign_errmsg                 %ld\n"
        "largest message offered                %d\n"
        "assignments with s.size() == cap       %ld     <- 10e6dfb3 needs > 0\n"
        "assignments with s.size() >  cap       %ld\n",
        calls, n_min, n_max, n_nonpositive, cap_min, cap_max, no_buffer,
        assigns, biggest_msg, size_eq_cap, size_gt_cap);
}
struct Install { Install() { std::atexit(report); } } install;
}  // namespace vitprobe
...
void assign_errmsg(errorvariables_view_t* ErrVar, std::string_view s) {
    vitprobe::assigns++;
    if (static_cast<int>(s.size()) > vitprobe::biggest_msg)
        vitprobe::biggest_msg = static_cast<int>(s.size());
    if (static_cast<int>(s.size()) == ErrVar->n_ErrMsg_cap) vitprobe::size_eq_cap++;
    if (static_cast<int>(s.size()) >  ErrVar->n_ErrMsg_cap) vitprobe::size_gt_cap++;
...
void unwrap(double* x, int n_x, errorvariables_view_t* ErrVar, double* unwrap_result) {
    ...
    vitprobe::calls++;
    if (ErrVar->n_ErrMsg < vitprobe::n_min) vitprobe::n_min = ErrVar->n_ErrMsg;
    if (ErrVar->n_ErrMsg > vitprobe::n_max) vitprobe::n_max = ErrVar->n_ErrMsg;
    if (ErrVar->n_ErrMsg <= 0) vitprobe::n_nonpositive++;
    if (ErrVar->n_ErrMsg_cap < vitprobe::cap_min) vitprobe::cap_min = ErrVar->n_ErrMsg_cap;
    if (ErrVar->n_ErrMsg_cap > vitprobe::cap_max) vitprobe::cap_max = ErrVar->n_ErrMsg_cap;
    ...
    for (int i = 2; i <= n_x; ++i) {
        ...
        while (y[i - 1] - y[i - 2] <= -PI) {
            shift_tail(y, i, n_x, TWO_PI);
        }
