...
#
#   bash evidence/wrap_180/run_wrapper_redtest.sh
#
...
#
# wrap_180's wrapper has no reverse-copy line to delete: its only dummy is
# INTENT(IN). What it does have is the ARGUMENT HANDOFF, so that is what is
# perturbed -- the sign of `x` on its way into `wrap_180_c`.
#
...
FROM='wrap_180_result = REAL(wrap_180_c(x), 8)'
TO='wrap_180_result = REAL(wrap_180_c(-x), 8)'
...
bash scripts/harness.sh wrap_180 Functions wrap_180 "$F" --post-integration \
     --out harness/wrap_180.postintegration.redtest.json \
     --red-test "the wrapper hands -x to wrap_180_c instead of x" 2>&1 | tail -6

restore; trap - EXIT
echo "--- reverted build in place; re-running the green"
bash scripts/harness.sh wrap_180 Functions wrap_180 "$F" --post-integration \
     --out evidence/wrap_180/harness.postintegration.revert-verified.json 2>&1 | tail -3
