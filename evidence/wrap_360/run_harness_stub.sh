...
#
#   bash evidence/wrap_360/run_harness_stub.sh <stub.cpp> <out.json> "<what was cut>"
#
...
# `translations/Functions/wrap_360.cpp` to `wrap_360.hpp`, so swapping that file is
# the whole of the lever.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"
STUB="$1"; OUT="$2"; WHAT="$3"
LIVE=translations/Functions/wrap_360.cpp
KEEP=$(mktemp)
cp "$LIVE" "$KEEP"
...
bash scripts/harness.sh wrap_360 Functions wrap_360 rosco/controller/src/Functions.f90 \
