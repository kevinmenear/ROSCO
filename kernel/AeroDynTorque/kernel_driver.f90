    ...
    PROGRAM kernel_driver 
        ...
        USE controllerblocks, ONLY: windspeedestimator 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 AeroDynTorque.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    CALL kr_rosco_types_performancedata(perfdata, kgen_unit, "perfdata", .FALSE.) 
                    CALL kr_rosco_types_errorvariables(errvar, kgen_unit, "errvar", .FALSE.) 
                      
                    !extern input variables 
                      
                    !callsite part 
                    CALL windspeedestimator(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, cntrpar, localvar, perfdata, &
                    &errvar) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: AeroDynTorque" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: AeroDynTorque: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: AeroDynTorque: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
