...
!Generated at : 2026-03-18 09:46:28 
!KGEN version : 0.8.1 
...
!-------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE ipc(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, cntrpar, localvar) 
    ...
    REAL(KIND=dbki) :: pitcomipc_1p(3) 
    REAL(KIND=dbki) :: axisyawipc_1p 
    ...
    REAL(KIND=dbki), dimension(3) :: kgenref_pitcomipc_1p 
    ...
    IF (kgen_istrue) THEN 
        ...
        READ (UNIT = kgen_unit) pitcomipc_1p 
        CALL kgen_array_sumcheck("pitcomipc_1p", kgen_array_sum, DBLE(SUM(pitcomipc_1p, mask=(pitcomipc_1p .eq. pitcomipc_1p))), &
        &.TRUE.) 
    END IF   
    READ (UNIT = kgen_unit) axisyawipc_1p 
    ...
    IF (kgen_istrue) THEN 
        READ (UNIT = kgen_unit) kgen_array_sum 
        READ (UNIT = kgen_unit) kgenref_pitcomipc_1p 
        CALL kgen_array_sumcheck("kgenref_pitcomipc_1p", kgen_array_sum, DBLE(SUM(kgenref_pitcomipc_1p, &
        &mask=(kgenref_pitcomipc_1p .eq. kgenref_pitcomipc_1p))), .TRUE.) 
    END IF   
        ...
        !$kgen begin_callsite ColemanTransformInverse
    ...
    !call to kgen kernel 
        ...
        IF (kgen_mainstage) THEN 
            ...
            CALL kv_ipc_real__dbki_dim1("pitcomipc_1p", check_status, pitcomipc_1p, kgenref_pitcomipc_1p) 
        ...
        CALL ColemanTransformInverse(LocalVar%IPC_axisTilt_1P, axisYawIPC_1P, LocalVar%Azimuth, NP_1, CntrPar%IPC_aziOffset(1), PitComIPC_1P)
            ...
            IF (check_status%rank==0) THEN 
                WRITE (*, *) "ColemanTransformInverse : Time per call (usec): ", kgen_measure 
            END IF   
        END IF   
        IF (kgen_warmupstage) THEN 
        END IF   
        ...
        !verify state subroutine for kv_ipc_real__dbki_dim1 
        RECURSIVE SUBROUTINE kv_ipc_real__dbki_dim1(varname, check_status, var, kgenref_var) 
        ...
        END SUBROUTINE kv_ipc_real__dbki_dim1 
