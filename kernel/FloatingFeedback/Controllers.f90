...
!Generated at : 2026-03-25 23:42:45 
!KGEN version : 0.8.1 
...
CONTAINS
!-------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE pitchcontrol(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, cntrpar, localvar, objinst, errvar) 
    ...
    !call to kgen kernel 
            ...
            LocalVar%Fl_PitCom = FloatingFeedback(LocalVar, CntrPar, objInst, ErrVar)
                ...
                IF (check_status%rank==0) THEN 
                    WRITE (*, *) "FloatingFeedback : Time per call (usec): ", kgen_measure 
                END IF   
            END IF   
            IF (kgen_warmupstage) THEN 
            END IF   
