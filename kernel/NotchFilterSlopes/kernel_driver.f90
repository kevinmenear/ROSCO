    ...
    !Generated at : 2026-03-25 23:44:13 
    !KGEN version : 0.8.1 
      
    PROGRAM kernel_driver 
        USE kgen_utils_mod
        USE tprof_mod, ONLY: tstart, tstop, tnull, tprnt 
        USE filters, ONLY: prefiltermeasuredsignals 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 NotchFilterSlopes.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    !callsite part 
                    CALL prefiltermeasuredsignals(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, localvar, objinst) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: NotchFilterSlopes" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: NotchFilterSlopes: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: NotchFilterSlopes: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
