...
CONTAINS
!-------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE pitchcontrol(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, localvar, objinst) 
    ...
    USE rosco_types, ONLY: localvariables, objectinstances 
        ! Inputs
    USE kgen_utils_mod
    USE kgen_utils_mod
    ...
    CALL kr_rosco_types_localvariables(kgenref_localvar, kgen_unit, "kgenref_localvar", .FALSE.) 
        ...
        !$kgen begin_callsite PIController
    ...
    !call to kgen kernel 
        ...
        LocalVar%PC_PitComT = PIController(LocalVar%PC_SpdErr, LocalVar%PC_KP, LocalVar%PC_KI, LocalVar%PC_MinPit, LocalVar%PC_MaxPit, LocalVar%DT, LocalVar%BlPitch(1), LocalVar%piP, (LocalVar%restart /= 0), objInst%instPI)
            ...
            IF (check_status%rank==0) THEN 
                WRITE (*, *) "PIController : Time per call (usec): ", kgen_measure 
            END IF   
        END IF   
        IF (kgen_warmupstage) THEN 
        END IF   
