    ...
    !Generated at : 2026-03-18 21:33:49 
    !KGEN version : 0.8.1 
    ...
    PROGRAM kernel_driver 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 SecLPFilter.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: SecLPFilter" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: SecLPFilter: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: SecLPFilter: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
