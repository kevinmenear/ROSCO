...
! VIT: removed invalid PUBLIC statement
    ...
    TYPE(performancedata) :: kgenref_perfdata 
    ...
    CALL kr_rosco_types_performancedata(kgenref_perfdata, kgen_unit, "kgenref_perfdata", .FALSE.) 
...
! Filter signals
    ...
    IF (kgen_mainstage) THEN 
        ...
        CALL kv_rosco_types_performancedata("perfdata", check_status, perfdata, kgenref_perfdata) 
    ...
    CALL WindSpeedEstimator(LocalVar, CntrPar, objInst, PerfData, DebugVar, ErrVar)
        ...
        IF (check_status%rank==0) THEN 
            WRITE (*, *) "WindSpeedEstimator : Time per call (usec): ", kgen_measure 
        END IF   
    END IF   
    IF (kgen_warmupstage) THEN 
    END IF   
