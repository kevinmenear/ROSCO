...
!Generated at : 2026-03-18 20:10:15 
!KGEN version : 0.8.1 
...
CONTAINS
!-------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE pitchcontrol(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, cntrpar, localvar, errvar) 
    ...
    !call to kgen kernel 
        ...
        LocalVar%PC_KP = interp1d(CntrPar%PC_GS_angles, CntrPar%PC_GS_KP, LocalVar%BlPitchCMeasF, ErrVar) ! Proportional gain
            ...
            IF (check_status%rank==0) THEN 
                WRITE (*, *) "interp1d : Time per call (usec): ", kgen_measure 
            END IF   
        END IF   
        IF (kgen_warmupstage) THEN 
        END IF   
