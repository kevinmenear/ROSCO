...
!Generated at : 2026-04-02 17:33:54 
!KGEN version : 0.8.1 
...
!-------------------------------------------------------------------------------------------------------------------------------  
SUBROUTINE variablespeedcontrol(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, localvar, errvar) 
    ...
    USE rosco_types, ONLY: localvariables, errorvariables 
        ! Inputs
    USE kgen_utils_mod
    USE kgen_utils_mod
    ...
    !call to kgen kernel 
                ...
                LocalVar%AzBuffer = UNWRAP(LocalVar%AzBuffer, ErrVar)
                    ...
                    IF (check_status%rank==0) THEN 
                        WRITE (*, *) "unwrap : Time per call (usec): ", kgen_measure 
                    END IF   
                END IF   
                IF (kgen_warmupstage) THEN 
                END IF   
