...
!-------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE prefiltermeasuredsignals(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath) 
    ...
    REAL(KIND=dbki) :: nacvanecosf 
    REAL(KIND=dbki) :: nacvanesinf 
    ...
    READ (UNIT = kgen_unit) nacvanecosf 
    READ (UNIT = kgen_unit) nacvanesinf 
    ...
    !call to kgen kernel 
        ...
        LocalVar%NacVaneF = wrap_180(atan2(NacVaneSinF, NacVaneCosF) * R2D) ! (deg)
            ...
            IF (check_status%rank==0) THEN 
                WRITE (*, *) "wrap_180 : Time per call (usec): ", kgen_measure 
            END IF   
        END IF   
        IF (kgen_warmupstage) THEN 
        END IF   
