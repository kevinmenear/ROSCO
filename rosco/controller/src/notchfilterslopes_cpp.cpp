// C++ implementation of Fortran function: NotchFilterSlopes
// Generated by VIT (Verified Incremental Translator)
// After verification: NotchFilterSlopes kernel PASSED

#include <algorithm>
#include <cmath>

// Auto-generated C struct for Fortran TYPE(FilterParameters)
// Generated by VIT (Verified Incremental Translator)

#ifndef FILTERPARAMETERS_T_H
#define FILTERPARAMETERS_T_H

typedef struct {
    double lpf1_a1[1024];
    double lpf1_a0[1024];
    double lpf1_b1[1024];
    double lpf1_b0[1024];
    double lpf1_InputSignalLast[1024];
    double lpf1_OutputSignalLast[1024];
    double lpf2_a2[1024];
    double lpf2_a1[1024];
    double lpf2_a0[1024];
    double lpf2_b2[1024];
    double lpf2_b1[1024];
    double lpf2_b0[1024];
    double lpf2_InputSignalLast2[1024];
    double lpf2_OutputSignalLast2[1024];
    double lpf2_InputSignalLast1[1024];
    double lpf2_OutputSignalLast1[1024];
    double lpfV_a2[1024];
    double lpfV_a1[1024];
    double lpfV_a0[1024];
    double lpfV_b2[1024];
    double lpfV_b1[1024];
    double lpfV_b0[1024];
    double lpfV_InputSignalLast2[1024];
    double lpfV_OutputSignalLast2[1024];
    double lpfV_InputSignalLast1[1024];
    double lpfV_OutputSignalLast1[1024];
    double hpf_InputSignalLast[1024];
    double hpf_OutputSignalLast[1024];
    double nfs_OutputSignalLast1[1024];
    double nfs_OutputSignalLast2[1024];
    double nfs_InputSignalLast1[1024];
    double nfs_InputSignalLast2[1024];
    double nfs_b2[1024];
    double nfs_b0[1024];
    double nfs_a2[1024];
    double nfs_a1[1024];
    double nfs_a0[1024];
    double nf_OutputSignalLast1[1024];
    double nf_OutputSignalLast2[1024];
    double nf_InputSignalLast1[1024];
    double nf_InputSignalLast2[1024];
    double nf_b2[1024];
    double nf_b1[1024];
    double nf_b0[1024];
    double nf_a1[1024];
    double nf_a0[1024];
} filterparameters_t;

#endif // FILTERPARAMETERS_T_H

// --- User-provided C++ implementation ---
// VIT Translation Scaffold
// Function: NotchFilterSlopes
// Source: Filters.f90
// Module: Filters
// Fortran: FUNCTION NotchFilterSlopes(InputSignal, DT, CornerFreq, Damp, FP, iStatus, reset, inst, Moving, InitialValue)
// Source MD5: af5e254dfd97
// VIT: 0.1.0
// Status: unverified
// Generated: 2026-03-19T03:19:26Z

// Auto-generated C struct for Fortran TYPE(FilterParameters)
// Generated by VIT (Verified Incremental Translator)

#ifndef FILTERPARAMETERS_T_H
#define FILTERPARAMETERS_T_H

typedef struct {
    double lpf1_a1[1024];
    double lpf1_a0[1024];
    double lpf1_b1[1024];
    double lpf1_b0[1024];
    double lpf1_InputSignalLast[1024];
    double lpf1_OutputSignalLast[1024];
    double lpf2_a2[1024];
    double lpf2_a1[1024];
    double lpf2_a0[1024];
    double lpf2_b2[1024];
    double lpf2_b1[1024];
    double lpf2_b0[1024];
    double lpf2_InputSignalLast2[1024];
    double lpf2_OutputSignalLast2[1024];
    double lpf2_InputSignalLast1[1024];
    double lpf2_OutputSignalLast1[1024];
    double lpfV_a2[1024];
    double lpfV_a1[1024];
    double lpfV_a0[1024];
    double lpfV_b2[1024];
    double lpfV_b1[1024];
    double lpfV_b0[1024];
    double lpfV_InputSignalLast2[1024];
    double lpfV_OutputSignalLast2[1024];
    double lpfV_InputSignalLast1[1024];
    double lpfV_OutputSignalLast1[1024];
    double hpf_InputSignalLast[1024];
    double hpf_OutputSignalLast[1024];
    double nfs_OutputSignalLast1[1024];
    double nfs_OutputSignalLast2[1024];
    double nfs_InputSignalLast1[1024];
    double nfs_InputSignalLast2[1024];
    double nfs_b2[1024];
    double nfs_b0[1024];
    double nfs_a2[1024];
    double nfs_a1[1024];
    double nfs_a0[1024];
    double nf_OutputSignalLast1[1024];
    double nf_OutputSignalLast2[1024];
    double nf_InputSignalLast1[1024];
    double nf_InputSignalLast2[1024];
    double nf_b2[1024];
    double nf_b1[1024];
    double nf_b0[1024];
    double nf_a1[1024];
    double nf_a0[1024];
} filterparameters_t;

#endif // FILTERPARAMETERS_T_H

double notchfilterslopes(double InputSignal, double DT, double CornerFreq, double Damp, filterparameters_t* FP, int iStatus, int reset, int* inst, int has_Moving, int Moving, int has_InitialValue, double InitialValue) {
    int idx = *inst - 1;  // Fortran 1-based → C 0-based

    // OPTIONAL handling
    double InitialValue_ = InputSignal;
    if (has_InitialValue) InitialValue_ = InitialValue;

    int Moving_ = 0;
    if (has_Moving) Moving_ = Moving;

    // Saturate corner frequency at 0
    double CornerFreq_ = (CornerFreq < 0.0) ? 0.0 : CornerFreq;

    // Initialize state
    if (iStatus == 0 || reset) {
        FP->nfs_OutputSignalLast1[idx] = InitialValue_;
        FP->nfs_OutputSignalLast2[idx] = InitialValue_;
        FP->nfs_InputSignalLast1[idx] = InitialValue_;
        FP->nfs_InputSignalLast2[idx] = InitialValue_;
    }

    // Compute/update coefficients (also on Moving)
    if (iStatus == 0 || reset || Moving_) {
        FP->nfs_b2[idx] = 2.0 * DT * CornerFreq_;
        FP->nfs_b0[idx] = -FP->nfs_b2[idx];
        FP->nfs_a2[idx] = Damp * DT * DT * CornerFreq_ * CornerFreq_ + 2.0 * DT * CornerFreq_ + 4.0 * Damp;
        FP->nfs_a1[idx] = 2.0 * Damp * DT * DT * CornerFreq_ * CornerFreq_ - 8.0 * Damp;
        FP->nfs_a0[idx] = Damp * DT * DT * CornerFreq_ * CornerFreq_ - 2.0 * DT * CornerFreq_ + 4.0 * Damp;
    }

    // Filter
    double result = 1.0 / FP->nfs_a2[idx] *
        (FP->nfs_b2[idx] * InputSignal
         + FP->nfs_b0[idx] * FP->nfs_InputSignalLast1[idx]
         - FP->nfs_a1[idx] * FP->nfs_OutputSignalLast1[idx]
         - FP->nfs_a0[idx] * FP->nfs_OutputSignalLast2[idx]);

    // Save signals for next time step
    FP->nfs_InputSignalLast2[idx] = FP->nfs_InputSignalLast1[idx];
    FP->nfs_InputSignalLast1[idx] = InputSignal;
    FP->nfs_OutputSignalLast2[idx] = FP->nfs_OutputSignalLast1[idx];
    FP->nfs_OutputSignalLast1[idx] = result;
    *inst = *inst + 1;

    return result;
}

// --- Fortran-callable C wrapper ---
extern "C" {
    double notchfilterslopes_c(double InputSignal, double DT, double CornerFreq, double Damp, filterparameters_t* FP, int iStatus, int reset, int* inst, int has_Moving, int Moving, int has_InitialValue, double InitialValue) {
        return notchfilterslopes(InputSignal, DT, CornerFreq, Damp, FP, iStatus, reset, inst, has_Moving, Moving, has_InitialValue, InitialValue);
    }
}
