...
CONTAINS
    ...
    SUBROUTINE vit_view_in_localvariables(view, dest)
        ...
        TYPE(localvariables_view_t), INTENT(IN) :: view
        TYPE(LOCALVARIABLES), INTENT(INOUT) :: dest
        CHARACTER(KIND=C_CHAR), POINTER :: vit_ap_acc_infile(:)
        ...
        ! --- Nested TYPE(rlParams) rlP ---
        dest%rlP%LastSignal = view%rlP%LastSignal
        IF (ALLOCATED(dest%ACC_INFILE)) DEALLOCATE(dest%ACC_INFILE)
        IF (C_ASSOCIATED(view%ACC_INFILE) .AND. view%n_ACC_INFILE > 0) THEN
            CALL C_F_POINTER(view%ACC_INFILE, vit_ap_acc_infile, [INT(view%n_ACC_INFILE)])
            ALLOCATE(dest%ACC_INFILE(view%n_ACC_INFILE))
            dest%ACC_INFILE = vit_ap_acc_infile
        ELSE IF (view%n_ACC_INFILE == 0) THEN
            ALLOCATE(dest%ACC_INFILE(0))   ! allocated-empty, distinct from unallocated
        END IF
    ...
    SUBROUTINE vit_view_out_localvariables(src, view)
        ...
        IF (C_ASSOCIATED(view%ACC_INFILE) .AND. view%n_ACC_INFILE > 0) THEN
            IF (ALLOCATED(src%ACC_INFILE)) THEN
                IF (SIZE(src%ACC_INFILE) == view%n_ACC_INFILE) THEN
                    CALL C_F_POINTER(view%ACC_INFILE, vit_ap_acc_infile, [INT(view%n_ACC_INFILE)])
                    vit_ap_acc_infile = src%ACC_INFILE
                ELSE
                    WRITE(ERROR_UNIT,'(A,I0,A,I0,A)') &
                        'VIT: LOCALVARIABLES%ACC_INFILE came back at ', SIZE(src%ACC_INFILE), &
                        ' element(s) against the ', view%n_ACC_INFILE, &
