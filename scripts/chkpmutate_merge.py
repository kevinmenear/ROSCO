#!/usr/bin/env python3
"""Union sliced/operator-filtered chkpmutate runs into one artifact, or refuse.
...
containing no C++ `WriteRestartFile` at all. Passing that check by renaming the field would
...
out of `chkpmutate.py`; this sums fields and recomputes the two derived numbers.

    python3 scripts/chkpmutate_merge.py --unit WriteRestartFile \\
        --cpp rosco/controller/src/writerestartfile.cpp \\
        --part mutation/WriteRestartFile.compare_op.0.json ... --out mutation/WriteRestartFile.json
"""
...
def die(msg: str) -> int:
    print(f"chkpmutate_merge: REFUSING -- {msg}", file=sys.stderr)
    return 2
...
def main() -> int:
    ...
    ap.add_argument("--unreachable", default=None,
                    help="JSON OBJECT {id: {reason, evidence}} of mutants "
                         "declared UNREACHABLE BY THIS CORPUS. `dbgmutate_merge` "
                         "has no such flag because `Debug` needed none; P12 "
                         "reads `unreachable_declared` and divides by "
                         "mutants - equivalent - unreachable, so a survivor "
                         "PROVED out of the corpus's reach leaves the ratio "
                         "instead of dragging it down. Checked HARDER than an "
                         "equivalence, because it is cheaper to assert: both "
                         "fields are required, the evidence path must EXIST, "
                         "and a declared mutant the sweep KILLED is refused.")
    ap.add_argument("--equivalences", default=None,
                    help="JSON {id: reason} of mutants declared EQUIVALENT. "
                         "Applied here rather than inside chkpmutate.py, for two "
    ...
    if a.unreachable:
        f = ROOT / a.unreachable
        if not f.is_file():
            return die(f"no such unreachable file: {a.unreachable}")
        unreachable = json.loads(f.read_text())
        if not isinstance(unreachable, dict):
            return die("--unreachable must be a JSON OBJECT {id: {reason, evidence}}")
        by_id = {r["mid"]: r for _, d in parts for r in d["results"]}
        for mid, rec in unreachable.items():
            if not isinstance(rec, dict) or not rec.get("reason") or not rec.get("evidence"):
                return die(f"unreachable {mid} needs both `reason` and `evidence`")
            if not (ROOT / rec["evidence"]).exists():
                return die(f"unreachable {mid} cites evidence that does not "
                           f"exist: {rec['evidence']}")
            if mid not in by_id:
                return die(f"declared unreachable but not in this sweep: {mid}")
            if by_id[mid]["killed"]:
                return die(f"declared UNREACHABLE and yet KILLED by this corpus: "
                           f"{mid} -- the declaration is wrong, not the sweep")
            if by_id[mid]["outcome"] == "nocompile":
                return die(f"declared unreachable but did not compile: {mid}")
            if mid in declared:
                return die(f"{mid} is declared BOTH equivalent and unreachable; "
                           f"they are different claims and it can be at most one")
    ...
    survivors_unreach = [s for s in survivors if s["id"] in unreachable]
    survivors = [s for s in survivors
                 if s["id"] not in declared and s["id"] not in unreachable]
    if len(survivors) + len(survivors_equiv) + len(survivors_unreach) != survived:
        return die("the survivor split lost a record")

    unreach = len(unreachable)
    denom = beh - eq - unreach
    if denom <= 0:
        return die(f"every one of the {beh} behavioural mutant(s) is declared "
                   f"({eq} equivalent, {unreach} unreachable) -- nothing was scored")
