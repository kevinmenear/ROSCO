...
if [ -f "$cal" ]; then
    python3 - "$cal" "$ROOT/$UNIT_DIR/Makefile" "$ROOT/$(dirname "$FFILE")" \
            "$MODE" "$ROOT/rosco/controller/build/CMakeFiles/discon.dir/src" <<'PYEOF'
import glob, os, re, sys
cal, mk, srcdir, mode, objdir = sys.argv[1:6]
text = open(cal).read()
mktext = open(mk).read()
...
for b in blocks:
    ...
    integrated = re.search(r"(?<![A-Za-z0-9_])%s_c\s*\(" % re.escape(name),
                           src) is not None
    # WHICH OBJECTS THE LINK WILL ACTUALLY CARRY, and the two modes differ.
    # Pre mode links the Makefile's own LIBS. POST mode adds every `.o` in the
    # build tree on its `make test LIBS=` command line, so an object absent from
    # the Makefile is still in the link -- which is how `chkparsedata_callees.o`
    # and the integrated `conv2uc.cpp.o`/`int2lstr.cpp.o` collided with
    # `multiple definition` the first time post mode ran after the pre-mode rule
    # started KEEPING bridges.
    linked = (f"/{name}.cpp.o" in mktext
              or (mode == "post"
                  and os.path.isfile(os.path.join(objdir, f"{name}.cpp.o"))))
    if integrated and linked:
        dropped.append(name)
    else:
        kept.append(b)
        if linked and mode != "post":
...
# THE SHIM'S SIGNATURE IS GENERATED, NOT TYPED. This heredoc used to carry
#
#     void ${UNIT}(double*, double, int, double*, double*);
#
# -- ColemanTransform's parameter list, as a literal, for every unit. Unit #1's
# `AddToList(CFI_cdesc_t*, int)` would have failed to compile, which is the good
# outcome; a unit whose arity happened to match would have compiled and
# forwarded the wrong arguments. `scripts/_integration_shim.py` asks VIT's
# `build_c_params` -- the generator that defines the ABI -- so there is one
