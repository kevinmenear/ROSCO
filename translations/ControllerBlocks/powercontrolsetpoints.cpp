...
// VIT: 0.1.0
// Status: unverified
// Generated: 2026-08-14T22:44:43Z
//
...
// its body -- the same standing PitchSaturation records one screen down in the
// same file. They are named here because the SIGNATURE is the contract, and
// left untouched because the reference leaves them untouched (P6).
//
// THE OUTER PREDICATE HAS TWO LIVE ARMS AND THE INNER ONE HAS THREE, OF WHICH
// ONE IS LIVE. Read from coverage/line_coverage.json, all 27 scenarios:
//
//   :38  IF (PRC_Mode == 2)          every scenario, 15,999 in 25
//   :39  IF (PRC_Comm == Constant)   SCENARIO 25 ONLY, 15,999
//   :44  ELSEIF (PRC_Comm == OpenLoop)   0 hits, all 27 scenarios
//   :65  ELSEIF (PRC_Comm == ZMQ)        0 hits, all 27 scenarios
//   :73  PRC_Min_Pitch = interp1d(...)   SCENARIO 25 ONLY, 15,999
//   :76  the ELSE arm                    every scenario EXCEPT 25
//
// So the gate compares one arm from scenario 25 and the other from the other
// 26, and the OpenLoop and ZMQ arms -- eleven of this unit's twenty statements
// -- are outside everything the simulation drives. Only the differential
// harness reaches them.
//
// AND ONE STATEMENT IS OUTSIDE EVERY INSTRUMENT IN THE CAMPAIGN, INCLUDING
// THAT ONE. `WRITE(401,*) LocalVar%PRC_R_Speed` writes a record to a Fortran
// unit nothing opens, closes or reads; plan.json calls it "an UNCOMPARED
// output, not a missing input". The differential harness compares the mapped
// signature and no signature carries a file. It is reproduced here anyway,
// because the mirror contract is about the reference's behaviour and not about
// the part of it some instrument happens to watch -- and because a statement
// dropped for being unobservable is exactly the class of defect this campaign
// keeps finding. What compares it is evidence/PowerControlSetpoints/ld_probe:
// the record format was MEASURED against 22,526 gfortran records, not read off
// a standard, and the probe is the artifact that says so.
