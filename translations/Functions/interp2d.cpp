...
double interp2d(double* xData, int n_xData, double* yData, int n_yData,
    ...
    if (xq <= xMin || std::isnan(xq)) {
        ...
        return interp1d_c(yData, n_yData, &zData[0], n_zData_rows, yq, ErrVar);
    } else if (xq >= xMax) {
        // On upper x-bound: interp1d on last column
        int last_col = n_xData - 1;
        return interp1d_c(yData, n_yData, &zData[last_col * n_zData_rows], n_zData_rows, yq, ErrVar);
    } else {
        jj = -1;
        for (j = 0; j < n_xData; j++) {
            if (xq == xData[j]) {
                // On axis: interp1d on this column
                return interp1d_c(yData, n_yData, &zData[j * n_zData_rows], n_zData_rows, yq, ErrVar);
            } else if (xq < xData[j]) {
                jj = j;
                break;
            }
    ...
    if (yq <= yMin || std::isnan(yq)) {
        ...
        result = interp1d_c(xData, n_xData, row_temp, n_xData, xq, ErrVar);
        delete[] row_temp;
        return result;
    } else if (yq >= yMax) {
