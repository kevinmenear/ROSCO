...
// Parse ONE real starting at `p`. On success returns true, writes the value and
// advances `p` to the first character past the number. On a malformed number
// returns false with `p` unspecified. The terminator is NOT checked here --
// that is the caller's job, because the two have different consequences (see
// the ';' and '1.0.0' rows above).
bool parse_real(const char* rec, int len, int& p, double& out) {
    const int start = p;
